-- ===================================================================
-- EMPLOYEE SCHEMA MIGRATION SCRIPT - SAFE VERSION
-- ===================================================================
-- This version handles errors gracefully without using transactions
-- Each operation is independent and will continue even if one fails
-- ===================================================================

\echo ''
\echo '═══════════════════════════════════════════════════════════════════'
\echo 'EMPLOYEE MANAGEMENT SCHEMA MIGRATION - SAFE VERSION'
\echo '═══════════════════════════════════════════════════════════════════'
\echo ''
...
DROP POLICY IF EXISTS "Employees manageable by admin and managers" ON employees;
DROP POLICY IF EXISTS "Attendance viewable by authenticated users" ON attendance_records;
DROP POLICY IF EXISTS "Employees can manage their own attendance" ON attendance_records;
DROP POLICY IF EXISTS "Admin and managers can manage all attendance" ON attendance_records;
DROP POLICY IF EXISTS "Leave requests viewable by authenticated users" ON leave_requests;
DROP POLICY IF EXISTS "Employees can create their own leave requests" ON leave_requests;
DROP POLICY IF EXISTS "Admin and managers can manage all leave requests" ON leave_requests;
DROP POLICY IF EXISTS "Shifts viewable by authenticated users" ON employee_shifts;
DROP POLICY IF EXISTS "Admin and managers can manage shifts" ON employee_shifts;
DROP POLICY IF EXISTS "Shift templates viewable by authenticated users" ON shift_templates;
DROP POLICY IF EXISTS "Admin can manage shift templates" ON shift_templates;
...
\echo '✅ Enabled RLS and created policies'
...
\echo ''
\echo 'Next steps:'
\echo '1. Run: \\i VERIFY-EMPLOYEE-SCHEMA.sql (to verify everything works)'
\echo '2. Navigate to /employees in your application'
