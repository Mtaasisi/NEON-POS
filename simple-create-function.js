...
async function createFunction() {
  ...
  try {
    ...
    // Create a simplified version of the function first
...
`;
    ...
    console.log('📝 Executing CREATE FUNCTION statement...');
    await sql.unsafe(createFunctionSQL);

    console.log('📝 Executing GRANT statement...');
    await sql.unsafe(`GRANT EXECUTE ON FUNCTION public.reverse_purchase_order_payment(UUID, UUID) TO authenticated;`);

    console.log('📝 Executing COMMENT statement...');
    await sql.unsafe(`COMMENT ON FUNCTION public.reverse_purchase_order_payment(UUID, UUID) IS 'Reverses a purchase order payment, restoring balances, removing related records, and updating the purchase order totals.';`);
    ...
    if (result.length > 0) {
      ...
      // Test the function with a non-existent payment to make sure it responds
      try {
        ...
        if (error.message && error.message.includes('Payment not found')) {
          console.log('✅ Function test successful (got expected error)');
        } else {
          console.log('⚠️ Function test returned unexpected result, but function exists');
        }
      }
    } else {
