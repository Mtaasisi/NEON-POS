...
async function checkProducts() {
  console.log('🔍 Checking lats_products in database...\n');
  ...
  try {
    ...
    const allProducts = await sql`
      SELECT 
        id, name, sku, selling_price, stock_quantity, is_active,
        branch_id, is_shared,
    ...
    if (allProducts.length === 0) {
      ...
      allProducts.forEach((prod, idx) => {
        ...
        console.log(`   Price: ${prod.selling_price}, Stock: ${prod.stock_quantity}`);
    ...
    if (branchOrShared.length > 0) {
      branchOrShared.forEach(p => {
        console.log(`   - ${p.name}: branch_id=${p.branch_id || 'NULL'}, is_shared=${p.is_shared}`);
      });
    }
    ...
    // 5. Check table structure for column issues
