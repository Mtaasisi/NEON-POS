...
/**
 * Simple script to fix the suppliers issue - Version 2
 * This creates real suppliers in the database without problematic columns
 */
...
async function fixSuppliers() {
  try {
    ...
    console.log('Table columns:', tableStructure.map(col => `${col.column_name} (${col.data_type})`).join(', '));
    ...
    // Create real suppliers with only existing columns
