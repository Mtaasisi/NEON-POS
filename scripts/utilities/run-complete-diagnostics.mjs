...
async function query8_CheckParentVariantStock() {
  ...
  try {
    ...
    if (parentStocks.length > 0) {
      ...
      console.log('Parent Variant Summary (Top 10):');
      console.log('─────────────────────────────────────────────────────────────────');
      parentStocks.slice(0, 10).forEach((p, idx) => {
        console.log(`${idx + 1}. ${p.variant_name}`);
...
async function query9_InventorySummary() {
  ...
  try {
    const summary = await sql`
      SELECT
        SUM(CASE WHEN variant_type = 'parent' THEN stock_quantity ELSE 0 END) AS parent_stock_total,
        SUM(CASE WHEN variant_type = 'imei_child' THEN stock_quantity ELSE 0 END) AS imei_stock_total,
  ...
  try {
    ...
    const stockMismatches = await sql`
      ...
      FROM (
        ...
        HAVING p.stock_quantity != COALESCE(SUM(c.stock_quantity), 0)
      ) as mismatches
    `;
