-- ============================================
-- CREATE ALL POS SETTINGS TABLES
-- Run this in your Neon database console
-- ============================================
-- This creates ALL 11 settings tables that your
-- posSettingsApi.ts expects
-- ============================================
...
-- General Settings
CREATE POLICY IF NOT EXISTS "Users can view own general settings" ON lats_pos_general_settings FOR SELECT USING (auth.uid() = user_id);
CREATE POLICY IF NOT EXISTS "Users can insert own general settings" ON lats_pos_general_settings FOR INSERT WITH CHECK (auth.uid() = user_id);
CREATE POLICY IF NOT EXISTS "Users can update own general settings" ON lats_pos_general_settings FOR UPDATE USING (auth.uid() = user_id);
CREATE POLICY IF NOT EXISTS "Users can delete own general settings" ON lats_pos_general_settings FOR DELETE USING (auth.uid() = user_id);

-- Dynamic Pricing Settings
CREATE POLICY IF NOT EXISTS "Users can view own pricing settings" ON lats_pos_dynamic_pricing_settings FOR SELECT USING (auth.uid() = user_id);
CREATE POLICY IF NOT EXISTS "Users can insert own pricing settings" ON lats_pos_dynamic_pricing_settings FOR INSERT WITH CHECK (auth.uid() = user_id);
CREATE POLICY IF NOT EXISTS "Users can update own pricing settings" ON lats_pos_dynamic_pricing_settings FOR UPDATE USING (auth.uid() = user_id);
CREATE POLICY IF NOT EXISTS "Users can delete own pricing settings" ON lats_pos_dynamic_pricing_settings FOR DELETE USING (auth.uid() = user_id);

-- Receipt Settings
CREATE POLICY IF NOT EXISTS "Users can view own receipt settings" ON lats_pos_receipt_settings FOR SELECT USING (auth.uid() = user_id);
CREATE POLICY IF NOT EXISTS "Users can insert own receipt settings" ON lats_pos_receipt_settings FOR INSERT WITH CHECK (auth.uid() = user_id);
CREATE POLICY IF NOT EXISTS "Users can update own receipt settings" ON lats_pos_receipt_settings FOR UPDATE USING (auth.uid() = user_id);
CREATE POLICY IF NOT EXISTS "Users can delete own receipt settings" ON lats_pos_receipt_settings FOR DELETE USING (auth.uid() = user_id);

-- Barcode Scanner Settings
CREATE POLICY IF NOT EXISTS "Users can view own scanner settings" ON lats_pos_barcode_scanner_settings FOR SELECT USING (auth.uid() = user_id);
CREATE POLICY IF NOT EXISTS "Users can insert own scanner settings" ON lats_pos_barcode_scanner_settings FOR INSERT WITH CHECK (auth.uid() = user_id);
CREATE POLICY IF NOT EXISTS "Users can update own scanner settings" ON lats_pos_barcode_scanner_settings FOR UPDATE USING (auth.uid() = user_id);
CREATE POLICY IF NOT EXISTS "Users can delete own scanner settings" ON lats_pos_barcode_scanner_settings FOR DELETE USING (auth.uid() = user_id);

-- Delivery Settings
CREATE POLICY IF NOT EXISTS "Users can view own delivery settings" ON lats_pos_delivery_settings FOR SELECT USING (auth.uid() = user_id);
CREATE POLICY IF NOT EXISTS "Users can insert own delivery settings" ON lats_pos_delivery_settings FOR INSERT WITH CHECK (auth.uid() = user_id);
CREATE POLICY IF NOT EXISTS "Users can update own delivery settings" ON lats_pos_delivery_settings FOR UPDATE USING (auth.uid() = user_id);
CREATE POLICY IF NOT EXISTS "Users can delete own delivery settings" ON lats_pos_delivery_settings FOR DELETE USING (auth.uid() = user_id);

-- Search Filter Settings
CREATE POLICY IF NOT EXISTS "Users can view own search settings" ON lats_pos_search_filter_settings FOR SELECT USING (auth.uid() = user_id);
CREATE POLICY IF NOT EXISTS "Users can insert own search settings" ON lats_pos_search_filter_settings FOR INSERT WITH CHECK (auth.uid() = user_id);
CREATE POLICY IF NOT EXISTS "Users can update own search settings" ON lats_pos_search_filter_settings FOR UPDATE USING (auth.uid() = user_id);
CREATE POLICY IF NOT EXISTS "Users can delete own search settings" ON lats_pos_search_filter_settings FOR DELETE USING (auth.uid() = user_id);

-- User Permissions Settings
CREATE POLICY IF NOT EXISTS "Users can view own permission settings" ON lats_pos_user_permissions_settings FOR SELECT USING (auth.uid() = user_id);
CREATE POLICY IF NOT EXISTS "Users can insert own permission settings" ON lats_pos_user_permissions_settings FOR INSERT WITH CHECK (auth.uid() = user_id);
CREATE POLICY IF NOT EXISTS "Users can update own permission settings" ON lats_pos_user_permissions_settings FOR UPDATE USING (auth.uid() = user_id);
CREATE POLICY IF NOT EXISTS "Users can delete own permission settings" ON lats_pos_user_permissions_settings FOR DELETE USING (auth.uid() = user_id);

-- Loyalty Customer Settings
CREATE POLICY IF NOT EXISTS "Users can view own loyalty settings" ON lats_pos_loyalty_customer_settings FOR SELECT USING (auth.uid() = user_id);
CREATE POLICY IF NOT EXISTS "Users can insert own loyalty settings" ON lats_pos_loyalty_customer_settings FOR INSERT WITH CHECK (auth.uid() = user_id);
CREATE POLICY IF NOT EXISTS "Users can update own loyalty settings" ON lats_pos_loyalty_customer_settings FOR UPDATE USING (auth.uid() = user_id);
CREATE POLICY IF NOT EXISTS "Users can delete own loyalty settings" ON lats_pos_loyalty_customer_settings FOR DELETE USING (auth.uid() = user_id);

-- Analytics Reporting Settings
CREATE POLICY IF NOT EXISTS "Users can view own analytics settings" ON lats_pos_analytics_reporting_settings FOR SELECT USING (auth.uid() = user_id);
CREATE POLICY IF NOT EXISTS "Users can insert own analytics settings" ON lats_pos_analytics_reporting_settings FOR INSERT WITH CHECK (auth.uid() = user_id);
CREATE POLICY IF NOT EXISTS "Users can update own analytics settings" ON lats_pos_analytics_reporting_settings FOR UPDATE USING (auth.uid() = user_id);
CREATE POLICY IF NOT EXISTS "Users can delete own analytics settings" ON lats_pos_analytics_reporting_settings FOR DELETE USING (auth.uid() = user_id);

-- Notification Settings
CREATE POLICY IF NOT EXISTS "Users can view own notification settings" ON lats_pos_notification_settings FOR SELECT USING (auth.uid() = user_id);
CREATE POLICY IF NOT EXISTS "Users can insert own notification settings" ON lats_pos_notification_settings FOR INSERT WITH CHECK (auth.uid() = user_id);
CREATE POLICY IF NOT EXISTS "Users can update own notification settings" ON lats_pos_notification_settings FOR UPDATE USING (auth.uid() = user_id);
CREATE POLICY IF NOT EXISTS "Users can delete own notification settings" ON lats_pos_notification_settings FOR DELETE USING (auth.uid() = user_id);

-- Advanced Settings
CREATE POLICY IF NOT EXISTS "Users can view own advanced settings" ON lats_pos_advanced_settings FOR SELECT USING (auth.uid() = user_id);
CREATE POLICY IF NOT EXISTS "Users can insert own advanced settings" ON lats_pos_advanced_settings FOR INSERT WITH CHECK (auth.uid() = user_id);
CREATE POLICY IF NOT EXISTS "Users can update own advanced settings" ON lats_pos_advanced_settings FOR UPDATE USING (auth.uid() = user_id);
CREATE POLICY IF NOT EXISTS "Users can delete own advanced settings" ON lats_pos_advanced_settings FOR DELETE USING (auth.uid() = user_id);
