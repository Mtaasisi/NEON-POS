...
const ShareReceiptModal: React.FC<ShareReceiptModalProps> = ({
  ...
  const generatePDF = async (forUpload: boolean = false): Promise<{ blob: Blob; url?: string } | null> => {
    try {
      ...
      // Skip direct data URL return for uploads - WhatsApp APIs don't support data: URLs
      if (isDevelopment && forUpload) {
        console.log('🚀 [DEVELOPMENT] For WhatsApp uploads, will try server upload first, then fallback to data URL processing');
      }
...
%%EOF`;
        ...
        console.log('✅ [DEVELOPMENT] Generated base64 PDF data URL for development mode');
        return {
