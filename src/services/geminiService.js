...
class GeminiService {
    ...
    requestCount = 0;
    lastRequestTime = 0;
    maxRequestsPerMinute = 1; // Reduced to 1 request per minute
    minRequestInterval = 60000; // Increased to 60 seconds
    // Service status tracking
    serviceAvailable = true;
    lastErrorTime = 0;
    errorCooldown = 300000; // Increased to 5 minutes
    consecutiveErrors = 0;
    maxConsecutiveErrors = 2; // Reduced to 2
    // Request queue for better rate limiting
    requestQueue = [];
    isProcessingQueue = false;
    // Static flag to track if warning has been shown
    static hasShownApiKeyWarning = false;
    constructor() {
    ...
    async generateCustomerResponse(customerQuery, context) {
        // Try AI first
    ...
    async generateInventoryResponse(query, productData) {
        // Try AI first
        const aiResponse = await this.chat([
            {
                role: 'user',
    ...
    async analyzeExpense(description, amount) {
        // Try AI first
        const aiResponse = await this.chat([
            {
                role: 'user',
    ...
    getFallbackCustomerResponse(query) {
    ...
    getFallbackInventoryResponse(query) {
        const lowerQuery = query.toLowerCase();
    ...
    getFallbackExpenseAnalysis(description, amount) {
...
Please consult with your accountant for specific tax advice.`;
    ...
    categorizeExpense(description) {
        ...
        if (lowerDesc.includes('travel') || lowerDesc.includes('transport'))
            return 'Travel';
        if (lowerDesc.includes('meal') || lowerDesc.includes('food'))
            return 'Meals & Entertainment';
        if (lowerDesc.includes('equipment') || lowerDesc.includes('hardware'))
            return 'Equipment';
