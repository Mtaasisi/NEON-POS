...
export const mediaLibraryService = {
  ...
  async upload(file: File, folder: string = 'General'): Promise<MediaLibraryItem> {
    console.log('📤 [MediaLibrary] Starting upload:', { 
      fileName: file.name, 
      fileType: file.type, 
      fileSize: file.size,
      folder 
    ...
    const fileName = `whatsapp-media/${folder}/${Date.now()}-${file.name}`;
    ...
    if (!publicUrl.startsWith('http://') && !publicUrl.startsWith('https://')) {
      ...
      if (supabaseUrl) {
        ...
        console.log('🔗 Converted to absolute URL:', fullUrl.substring(0, 100) + '...');
      } else {
        throw new Error('Cannot construct absolute URL: VITE_SUPABASE_URL not configured');
      }
    }
  ...
  async getAll(folder?: string): Promise<MediaLibraryItem[]> {
    ...
    // Ensure tags is always an array (handle NULL from database)
