...
export interface UserDailyGoal {
  id: string;
  user_id: string;
  date?: string; // Optional date field for compatibility
...
export interface CreateUserGoalData {
  ...
  goal_value: number;
  date?: string; // Optional date field for compatibility with date-based schemas
}
...
/**
 * Get all daily goals for a specific user (for today)
 */
export async function getUserDailyGoals(userId: string): Promise<UserDailyGoal[]> {
  try {
    // Get today's date in YYYY-MM-DD format for date-based schemas
    ...
    const { data, error } = await supabase
      ...
      .select('*')
      .eq('user_id', userId)
      .eq('date', today) // Only get today's goals
      .eq('is_active', true)
      .order('goal_type');
...
/**
 * Get a specific daily goal for a user (for today)
 */
export async function getUserDailyGoal(userId: string, goalType: UserDailyGoal['goal_type']): Promise<UserDailyGoal | null> {
  try {
    ...
    const { data, error } = await supabase
      ...
      .eq('date', today) // Only get today's goal
      .eq('is_active', true)
      .single();
...
export async function createUserDailyGoal(goalData: CreateUserGoalData): Promise<UserDailyGoal | null> {
  try {
    // Get today's date for checking existing goals (in YYYY-MM-DD format)
    const today = goalData.date || new Date().toISOString().split('T')[0];
    
    // First check if a goal of this type already exists for this user today
    const { data: existingGoal } = await supabase
      .from('user_daily_goals')
      .select('*')
      .eq('user_id', goalData.user_id)
      .eq('goal_type', goalData.goal_type)
      .eq('date', today) // Check for today's goal specifically
      .eq('is_active', true)
      .single();
    ...
    // Create new goal - add today's date if not provided (for date-based schemas)
    const goalDataWithDate = {
      ...goalData,
      date: goalData.date || new Date().toISOString().split('T')[0] // Add today's date in YYYY-MM-DD format
    };
    
    const { data, error } = await supabase
      .from('user_daily_goals')
      .insert(goalDataWithDate)
      .select()
      .single();
