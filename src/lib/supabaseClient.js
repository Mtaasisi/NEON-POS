...
if (typeof window !== 'undefined') {
    ...
    const isWebSocketError = (errorMsg, fullMessage, allArgsString) => {
    ...
    console.warn = (...args) => {
        ...
        if (isWebSocketError(warnMsg, fullMessage, allArgsString) ||
...
try {
    ...
    pool.on('error', (err) => {
        ...
        const isWebSocketError = (errorMsg.includes('WebSocket') ||
...
const executeSql = async (query, params = [], suppressLogs = false, retryCount = 0) => {
...
class NeonQueryBuilder {
    ...
    select(fields = '*', options) {
        ...
        if (fields.includes('(') && fields.includes(')')) {
            ...
            const extractColumns = (columnsStr) => {
            ...
            while ((match = inferredPattern.exec(workingFields)) !== null) {
                ...
                if (!foreignKey) {
                    ...
                    const isChildTable = knownChildPatterns.some(pattern => alias.includes(pattern) || table.includes(`_${pattern}`));
                    if (isChildTable) {
    ...
    single() {
        ...
        return this.then((result) => {
            ...
            const isInsertOp = this.isInsert;
            const isDevelopment = import.meta.env.DEV || import.meta.env.MODE === 'development';
    ...
    buildQuery() {
        ...
        for (const join of this.joins) {
            ...
            const isChildTable = knownChildPatterns.some(pattern => join.alias.includes(pattern) ||
    ...
    async execute() {
        ...
        try {
            ...
            const errorCode = error?.code || '';
            const isExpectedError = catchErrorMsg.includes('does not exist') ||
