...
export async function initializeSampleData(): Promise<void> {
  ...
  try {
    ...
    // Sample users - using the exact emails that AuthContext recognizes
    const users = [
      { id: 'user-1', email: 'admin@pos.com', password: 'admin123', name: 'Admin User', role: 'admin', branch_id: 'branch-1' },
      { id: 'user-2', email: 'care@care.com', password: '123456', name: 'Customer Care', role: 'customer-care', branch_id: 'branch-1' },
      { id: 'user-3', email: 'tech@pos.com', password: 'tech123', name: 'Technician', role: 'technician', branch_id: 'branch-2' },
      { id: 'user-4', email: 'manager@pos.com', password: 'manager123', name: 'Store Manager', role: 'manager', branch_id: 'branch-1' }
    ];
    ...
    const operations = [
      ...
      ...users.map(user => ({
        sql: `INSERT OR REPLACE INTO users (id, email, password, name, role, branch_id, is_active) VALUES (?, ?, ?, ?, ?, ?, 1)`,
        params: [user.id, user.email, user.password, user.name, user.role, user.branch_id]
      })),

      // Insert customers (depends on branches) - both tables for compatibility
      ...customers.map(cust => ({
      ...
      ...customers.map(cust => ({
        sql: `INSERT OR REPLACE INTO customers (id, name, phone, email, branch_id, is_active, loyalty_points, total_purchases) VALUES (?, ?, ?, ?, ?, 1, 0, 0.00)`,
        params: [cust.id, cust.name, cust.phone, cust.email, cust.branch_id]
      })),
