  ...
  async getExpenses(): Promise<ExpenseData[]> {
    try {
      ...
      if (accountIds.length > 0) {
        ...
        if (accountsData) {
          accountsMap = Object.fromEntries(accountsData.map(a => [a.id, a.name]));
        }
      }

      return data.map((expense: any) => ({
        ...expense,
        account_name: accountsMap[expense.account_id] || undefined
      }));
    } catch (error) {
  ...
  async getTransfers(): Promise<TransferData[]> {
    try {
      ...
      data?.forEach((transfer: any) => {
        if (transfer.from_account_id) accountIds.add(transfer.from_account_id);
        if (transfer.to_account_id) accountIds.add(transfer.to_account_id);
      });
      ...
      return data?.map((transfer: any) => ({
        ...transfer,
        from_account_name: accountsMap.get(transfer.from_account_id) || undefined,
        to_account_name: accountsMap.get(transfer.to_account_id) || undefined
      })) || [];
    } catch (error) {
  ...
  async getFinancialDataForPeriod(startDate: string, endDate: string): Promise<FinancialAnalytics | null> {
    try {
      ...
      if (payments && payments.length > 0) {
        ...
        transformedPayments = payments.map((payment: any) => {
          const device = devicesMap[payment.device_id];
          const customer = customersMap[payment.customer_id];
          return {
            ...payment,
      ...
      // Fetch related data for expenses
      let transformedExpenses: any[] = [];
      if (expenses && expenses.length > 0) {
        const accountIds = [...new Set(expenses.map(e => e.account_id).filter(Boolean))];
        let accountsMap: Record<string, string> = {};
        ...
        transformedExpenses = expenses.map((expense: any) => ({
          ...expense,
          account_name: accountsMap[expense.account_id] || undefined
        }));
      }
