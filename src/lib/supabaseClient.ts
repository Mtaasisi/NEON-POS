    ...
    upload: async (path: string, file: File | Blob, options?: any) => {
      try {
        ...
        if (result.success && result.url) {
          ...
          sessionStorage.setItem(`storage:${bucket}:${path}`, result.url);
    ...
    remove: async (paths: string[]) => {
      try {
        ...
        for (const path of paths) {
          const url = sessionStorage.getItem(`storage:${bucket}:${path}`);
          if (url) {
            await WhatsAppMediaStorageService.deleteMedia(url);
            sessionStorage.removeItem(`storage:${bucket}:${path}`);
          }
        }
