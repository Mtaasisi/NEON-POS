...
const ProductCard: React.FC<ProductCardProps> = ({
  ...
  // Load product images dynamically (only if not already provided)
  useEffect(() => {
    const loadImages = async () => {
      if (!product?.id) return;
      
      // If product already has images array, use those
      if (product.images && Array.isArray(product.images) && product.images.length > 0) {
        // Convert string URLs to ProductImage format
        const convertedImages: ProductImage[] = product.images.map((url, index) => ({
          id: `static-${index}`,
          url: typeof url === 'string' ? url : url.url || url.image_url,
          thumbnailUrl: typeof url === 'string' ? url : url.thumbnailUrl || url.thumbnail_url || url.url || url.image_url,
  ...
  const totalStock = childVariants.reduce((sum, v) => sum + v.stockQuantity, 0);
