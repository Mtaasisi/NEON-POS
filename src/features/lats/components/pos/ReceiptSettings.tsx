...
const ReceiptSettings: React.FC = () => {
  ...
  const loadCurrentSettings = async () => {
    setIsLoading(true);
    try {
      const settings = await POSSettingsService.loadReceiptSettings();
      if (settings) {
        // Map database fields to form fields
        reset({
          showLogo: settings.show_business_logo ?? true,
          showTax: settings.show_tax ?? true,
          showDiscount: settings.show_discounts ?? true,
          showCustomerInfo: settings.show_customer_name ?? true,
          showBarcode: settings.show_product_barcodes ?? true,
          showCashierInfo: settings.show_cashier_name ?? true,
          headerText: 'Welcome to Our Store',
          footerText: settings.footer_message ?? 'Thank you for your purchase!',
          companyName: 'Your Company Name',
          companyAddress: '123 Business Street, City',
          companyPhone: '+255 123 456 789',
          companyEmail: 'info@yourcompany.com',
          ...
          printCopies: 1,
          paperSize: settings.receipt_width === 80 ? '80mm' : '58mm',
          receiptPrefix: settings.receipt_number_prefix ?? 'RCP',
          receiptNumbering: settings.enable_receipt_numbering ?? true,
          startNumber: settings.receipt_number_start ?? 1,
          resetDaily: true,
          keepHistory: true,
          historyDays: 30,
          autoBackup: false,
  ...
  const handleSaveSettings = async (data: ReceiptSettings) => {
    ...
    try {
      ...
      const dbSettings = {
        receipt_template: 'standard' as 'standard' | 'compact' | 'detailed' | 'custom',
        receipt_width: data.paperSize === '80mm' ? 80 : 58,
        receipt_font_size: 12,
        show_business_logo: data.showLogo,
        ...
        show_date_time: true,
        show_cashier_name: data.showCashierInfo,
        show_customer_name: data.showCustomerInfo,
        show_customer_phone: false,
        show_product_names: true,
        show_product_skus: false,
        show_product_barcodes: data.showBarcode,
        show_quantities: true,
        show_unit_prices: true,
        show_discounts: data.showDiscount,
        show_subtotal: true,
        show_tax: data.showTax,
        show_discount_total: data.showDiscount,
        show_grand_total: true,
        show_payment_method: true,
        show_change_amount: true,
        auto_print_receipt: data.autoPrint,
        print_duplicate_receipt: data.printCopies > 1,
        enable_email_receipt: false,
        enable_sms_receipt: false,
        enable_receipt_numbering: data.receiptNumbering,
        receipt_number_prefix: data.receiptPrefix,
        receipt_number_start: data.startNumber,
