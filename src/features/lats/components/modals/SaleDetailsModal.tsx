...
const SaleDetailsModal: React.FC<SaleDetailsModalProps> = ({ isOpen, onClose, saleId }) => {
  ...
  const fetchSaleDetails = async () => {
    try {
      ...
      if (saleData.sold_by) {
        try {
          ...
          if (!userError && userData) {
            setCashierName(userData.full_name || userData.email || saleData.sold_by);
          } else {
            // Extract name from email if user not found
            const nameFromEmail = saleData.sold_by.split('@')[0].replace(/[._]/g, ' ');
            setCashierName(nameFromEmail.charAt(0).toUpperCase() + nameFromEmail.slice(1));
          }
        } catch (err) {
          console.warn('⚠️ Could not fetch cashier name:', err);
          const nameFromEmail = saleData.sold_by.split('@')[0].replace(/[._]/g, ' ');
          setCashierName(nameFromEmail.charAt(0).toUpperCase() + nameFromEmail.slice(1));
        }
      ...
      if (!combinedSaleData.discount && combinedSaleData.subtotal && combinedSaleData.total_amount) {
        const calculatedDiscount = combinedSaleData.subtotal - combinedSaleData.total_amount;
        if (calculatedDiscount > 0) {
          combinedSaleData.discount = calculatedDiscount;
      ...
      console.log('🔍 Financial data debug:', {
        ...
        discount: combinedSaleData.discount,
  ...
  const getPaymentMethodDisplay = (paymentMethod: any) => {
    ...
    if (typeof paymentMethod === 'string') {
      try {
        ...
        // Check for payments array regardless of type
        if (parsed.details?.payments && Array.isArray(parsed.details.payments) && parsed.details.payments.length > 1) {
          ...
          return `Multiple: ${uniqueMethods.join(' + ')}`;
        }
    ...
    if (typeof paymentMethod === 'object') {
      // Check for payments array regardless of type
      if (paymentMethod.details?.payments && Array.isArray(paymentMethod.details.payments) && paymentMethod.details.payments.length > 1) {
  ...
  const getPaymentMethodDetails = (paymentMethod: any) => {
    ...
    if (typeof paymentMethod === 'string') {
      try {
        const parsed = JSON.parse(paymentMethod);
        // Check for payments array in details, regardless of type
        if (parsed.details?.payments && Array.isArray(parsed.details.payments)) {
          // Only show breakdown if there are multiple payments
          return parsed.details.payments.length > 1 ? parsed.details.payments : null;
        }
        return null;
      } catch {
        return null;
    ...
    if (typeof paymentMethod === 'object') {
      ...
      if (paymentMethod.details?.payments && Array.isArray(paymentMethod.details.payments)) {
        // Only show breakdown if there are multiple payments
        return paymentMethod.details.payments.length > 1 ? paymentMethod.details.payments : null;
      }
      return null;
    }
                  ...
                  <div className="bg-white border border-gray-200 rounded-lg p-4">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                      ...
                      <div>
                        ...
                        <p className="text-gray-900">{cashierName}</p>
                      </div>
                      <div>
                ...
                <div className="space-y-4">
                  ...
                  <div className="bg-white border border-gray-200 rounded-lg p-4">
                    ...
                    <div className="space-y-3">
                      ...
                      {sale.discount === 0 && sale.subtotal && sale.total_amount && (
