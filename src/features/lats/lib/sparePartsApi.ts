...
export const getSpareParts = async (
  ...
  try {
    ...
    if (filters.search) {
      // Use RPC function for search to avoid .or() syntax issues with Neon
      ...
      if (searchError) {
        console.error('Error searching spare parts:', searchError);
        throw searchError;
      }
      ...
      // Get count from first result
      const totalCount = searchResults[0]?.total_count || 0;
      const sparePartIds = searchResults.map(r => r.id);
      
      // Fetch full details with relationships for the search results
      query = query.in('id', sparePartIds);
...
export const searchSpareParts = async (searchTerm: string): Promise<SparePart[]> => {
  try {
    // Use RPC function for Neon compatibility (avoids .or() syntax issues)
    const { data: searchResults, error: searchError } = await supabase
      .rpc('search_spare_parts_fn', {
    ...
    if (searchError) {
      console.error('Error searching spare parts via RPC:', searchError);
      throw searchError;
    }
