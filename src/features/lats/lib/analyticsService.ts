  ...
  static async getInventoryAnalytics(): Promise<InventoryAnalytics> {
    try {
      ...
      const [productsResult, variantsData] = await Promise.allSettled([
        ...
        supabase
          .from('lats_product_variants')
          .select('id, variant_name, product_id, quantity, cost_price, unit_price')
      ]);

      if (productsResult.status === 'rejected') throw productsResult.reason;
      
      const productsRaw = productsResult.value.data || [];
      const variants = variantsData.status === 'fulfilled' ? (variantsData.value.data || []) : [];
