...
async function fixInvalidUUIDs() {
  console.log('🔧 Fixing Invalid UUIDs in Customers\n');

  const uuidColumns = ['referred_by', 'created_by', 'branch_id', 'created_by_branch_id', 'id'];
  ...
  for (const col of uuidColumns) {
    try {
      const result = await targetPool.query(`
        ...
        AND ${col}::text NOT SIMILAR TO '[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}'
      `);
      
      const invalidCount = parseInt(result.rows[0].count);
      if (invalidCount > 0) {
        console.log(`   ⚠️  ${col}: ${invalidCount} invalid UUIDs found`);
        
        // Set to NULL if not a valid UUID (except for id which is primary key)
        if (col !== 'id') {
        ...
        console.log(`   ✅ ${col}: All valid`);
      }
    } catch (error) {
      // Column might not exist or might not be UUID type
      console.log(`   ⚠️  ${col}: ${error.message}`);
    }
  }

  // Now recreate function with proper NULL handling
  console.log('\n🔧 Recreating function with proper NULL handling...');
...
BEGIN
  ...
  SELECT
    ...
    CASE WHEN c.branch_id::text ~ '^[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}$' THEN c.branch_id ELSE NULL END::uuid as branch_id,
    COALESCE(c.is_shared, false)::boolean as is_shared,
    CASE WHEN c.created_by_branch_id::text ~ '^[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}$' THEN c.created_by_branch_id ELSE NULL END::uuid as created_by_branch_id,
    ...
    CASE WHEN c.referred_by::text ~ '^[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}$' THEN c.referred_by ELSE NULL END::uuid as referred_by,
    CASE WHEN c.created_by::text ~ '^[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}$' THEN c.created_by ELSE NULL END::uuid as created_by,
