...
console.log('\n📦 Applying Purchase Order Expense Tracking Fix...\n');
...
try {
  ...
  console.log(`\n   ✅ Successfully backfilled ${successCount} out of ${payments.length} payments\n`);
  ...
  const recentExpenses = await sql`
    ...
    WHERE fe.expense_category_id = ${categoryId}
  ...
  if (recentExpenses.length > 0) {
    console.log(`\n📊 Recent PO Payment Expenses:`);
    recentExpenses.forEach(exp => {
      console.log(`   ${exp.title}: ${exp.amount} (${exp.vendor})`);
    });
  }
  
  console.log(`\n🔄 Trigger active: All future PO payments will automatically create expenses`);
  console.log(`💰 All payments will check account balance before processing`);
  console.log(`📊 All payments will be tracked in the expenses table`);
  console.log(`═══════════════════════════════════════════════════════\n`);
