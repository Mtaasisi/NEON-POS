...
-- 3. Check for purchase order payments (if any)
SELECT 
  '📦 PURCHASE ORDER PAYMENTS' as check_type,
  pop.id,
  pop.amount,
  pop.currency,
  pop.payment_method,
...
SELECT 
  '📈 SUMMARY' as info,
  (SELECT COUNT(*) FROM finance_accounts WHERE is_payment_method = TRUE) as payment_methods_count,
  ...
  (SELECT COUNT(*) FROM purchase_order_payments) as po_payments_count;
...
BEGIN
  ...
  IF (SELECT COUNT(*) FROM finance_accounts WHERE is_payment_method = TRUE) = 0 THEN
    RAISE NOTICE '⚠️  No payment methods configured!';
    RAISE NOTICE '   Run: CREATE-PAYMENT-METHODS.sql';
  ELSE
    RAISE NOTICE '✅ Payment methods configured: %', (SELECT COUNT(*) FROM finance_accounts WHERE is_payment_method = TRUE);
  END IF;
