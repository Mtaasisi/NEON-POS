...
class ProductCreationTester {
  ...
  async fillProductForm(testData) {
    ...
    try {
      ...
      await this.page.waitForSelector('text=Product Information', { timeout: 15000 }).catch(() => {});
      ...
      if (testData.name) {
        ...
        const nameInput = this.page.locator('#product-name, input[name="name"], input[placeholder*="Product name" i]').first();
        // Ensure visible
        await nameInput.scrollIntoViewIfNeeded({ timeout: 8000 }).catch(() => {});
        // Try focusing via label
        const label = this.page.locator('label[for="product-name"], text=Product Name');
        if (await label.count()) {
          await label.first().click({ timeout: 5000 }).catch(() => {});
        }
        // Try direct click/fill
        await nameInput.click({ timeout: 8000 }).catch(() => {});
        await nameInput.fill('');
        await nameInput.type(testData.name, { delay: 15 }).catch(async () => {
          // Fallback: focus via JS and type (plain JS, no TS casts)
          ...
          await this.page.keyboard.type(testData.name, { delay: 15 });
        });
