...
class DashboardService {
  ...
  private async getDeviceStats() {
    try {
      ...
      if (error) {
        console.error('Device stats error details:', error);
        throw error;
      }
      ...
      console.error('Error fetching device stats:', {
  ...
  private async getPaymentStats() {
    try {
      ...
      if (error) {
        console.error('Payment stats error details:', error);
        throw error;
      }
      ...
      console.error('Error fetching payment stats:', error instanceof Error ? error.message : error);
  ...
  private async getInventoryStats() {
    try {
      ...
      console.error('Error fetching inventory stats:', error instanceof Error ? error.message : error);
  ...
  async getRecentActivity(limit: number = 20): Promise<RecentActivity[]> {
    try {
      ...
      const { data: devicesData } = await supabase
        .from('devices')
        .select('id, customer_name, status, service_type, created_at, updated_at')
      ...
      if (devicesData) {
        devicesData.forEach((device: any) => {
          activities.push({
            ...
            description: `${device.service_type || 'Service'} for ${device.customer_name}`,
      ...
      console.error('Error fetching recent activity:', error instanceof Error ? error.message : error);
      return [];
    }
  }
  ...
  async getFinancialSummary(): Promise<FinancialSummary> {
    try {
      ...
      console.error('Error fetching financial summary:', error instanceof Error ? error.message : error);
      return {
        todayRevenue: 0,
        weeklyRevenue: 0,
        monthlyRevenue: 0,
  ...
  async getInventoryAlerts(limit: number = 10): Promise<InventoryAlert[]> {
    try {
      ...
      console.error('Error fetching inventory alerts:', error instanceof Error ? error.message : error);
      return [];
    }
  }
  ...
  async getAnalyticsData(userId: string): Promise<AnalyticsData> {
    try {
      ...
      console.error('Error fetching analytics data:', error instanceof Error ? error.message : error);
      return {
        revenueGrowth: 0,
        customerGrowth: 0,
        averageOrderValue: 0,
