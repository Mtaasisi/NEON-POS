...
export const AuthProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  const fetchAndSetUserProfile = async (user: any) => {
    try {
      ...
      // If not found in auth_users table by ID, try by email
      ...
      // If still not found, try the users table by email
      if (!profileData) {
        ...
        if (!usersError && usersTableData) {
          console.log('Found user in users table:', usersTableData);
          // Map users table fields to auth_users format
          profileData = {
            id: usersTableData.id || user.id,
            email: usersTableData.email,
            username: (usersTableData.email as string)?.split('@')[0] || 'user',
            name: usersTableData.full_name || (usersTableData.email as string)?.split('@')[0] || 'User',
            role: usersTableData.role,
            is_active: usersTableData.is_active,
            created_at: usersTableData.created_at,
            updated_at: usersTableData.updated_at
          };
        }
      }

      // Only create default profile if not found anywhere
      if (!profileData) {
