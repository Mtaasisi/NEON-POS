...
// Helper function to execute SQL queries with the new Neon API
// Note: This uses a workaround for dynamic queries
// For better security, prefer using sql`` tagged template directly
const executeSql = async (query: string, params: any[] = []): Promise<any> => {
  try {
    // Neon serverless doesn't support string-based queries well
    // We need to use the tagged template syntax
    // For now, we'll use a workaround with neon's http endpoint
    
    // Get the connection string parts
    const connectionString = import.meta.env.VITE_DATABASE_URL;
    if (!connectionString) {
      throw new Error('VITE_DATABASE_URL is not configured');
    }
    
    // Extract the API endpoint from the connection string
    // Format: postgresql://user:pass@host/database or wss://...
    const match = connectionString.match(/(?:postgresql|wss):\/\/[^@]+@([^/]+)/);
    if (!match) {
      throw new Error('Invalid database URL format');
    }
    
    const host = match[1];
    const apiUrl = `https://${host.replace('.cloud.neon.tech', '.neon.tech')}/sql`;
    
    // Make HTTP request to Neon's SQL API
    const response = await fetch(apiUrl, {
      method: 'POST',
      headers: {
        'Content-Type': 'application/json',
        'Authorization': `Bearer ${connectionString.split(':')[2].split('@')[0]}` // Extract password as token
      },
      body: JSON.stringify({
        query: query,
        params: params
