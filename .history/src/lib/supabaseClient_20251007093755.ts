...
// Helper function to execute raw SQL queries via HTTP
async function executeRawSQL(query: string): Promise<any> {
  try {
    // Extract connection details from database URL
    const url = new URL(databaseUrl || '');
    const endpoint = url.hostname.replace('.neon.tech', '');
    const [region, ...rest] = endpoint.split('.');
    const apiUrl = `https://${endpoint}.neon.tech/sql`;
...
class NeonQueryBuilder {
  ...
  async then(resolve?: any, reject?: any) {
    try {
      ...
      // Execute using Neon serverless client (browser-compatible)
...
const rpcCall = async (fn: string, params?: any) => {
  try {
    ...
    // Execute using HTTP API for browser compatibility
