...
// The new API requires using tagged template literals
const executeSql = async (query: string): Promise<any> => {
  try {
    // Use the query as a raw template string
    // This creates a template strings array that sql() expects
  ...
  updateUser: async (attributes: any) => {
    try {
      ...
      const query = `
        ...
        RETURNING id, email, full_name, role, is_active, created_at, updated_at
      `;
      // Execute using Neon serverless client with new API (tagged template)
...
const rpcCall = async (fn: string, params?: any) => {
  try {
    ...
    // Execute using Neon serverless client with new API (tagged template)
    const data = await sql([query] as any);
    return { data, error: null };
  } catch (error: any) {
