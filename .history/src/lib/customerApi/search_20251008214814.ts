...
export async function searchCustomers(query: string, page: number = 1, pageSize: number = 50) {
  try {
    ...
    // Create optimized search conditions (using PostgREST wildcard syntax)
...
export async function searchCustomersFast(query: string, page: number = 1, pageSize: number = 50) {
  try {
    ...
    //  Build a proper SQL-compatible OR query using Supabase filter syntax
    const { data, error, count } = await supabase
      .from('customers')
      .select(`
        id,
        ...
        updated_at
      `, { count: 'exact' })
      .or(`name.ilike.%${query}%,phone.ilike.%${query}%,email.ilike.%${query}%,whatsapp.ilike.%${query}%,city.ilike.%${query}%,referral_source.ilike.%${query}%,customer_tag.ilike.%${query}%,initial_notes.ilike.%${query}%`)
      .range(offset, offset + pageSize - 1)
      .order('created_at', { ascending: false });
