...
export async function searchCustomers(query: string, page: number = 1, pageSize: number = 50) {
  try {
    ...
    if (searchError || !searchResults) {
      ...
      const { data: directResults, error: directError, count } = await supabase
        .from('customers')
      ...
      searchResults = directResults?.map(r => ({ ...r, total_count: count || 0 })) || [];
    }
...
export async function searchCustomersFast(query: string, page: number = 1, pageSize: number = 50) {
  try {
    ...
    // Use the custom search function created in Neon database
    const { data, error } = await supabase
      .rpc('search_customers_fn', {
        search_query: query,
        page_number: page,
