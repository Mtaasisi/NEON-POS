    ...
    if (data) {
      ...
      const processedCustomers = data.map(customer => {
        ...
        const mappedCustomer = {
          ...
          whatsapp: undefined, // Field doesn't exist in database yet
          whatsappOptOut: false, // Field doesn't exist in database yet
          ...
          createdBy: undefined, // Field doesn't exist in database yet
          lastPurchaseDate: undefined, // Field doesn't exist in database yet
          totalPurchases: 0, // Field doesn't exist in database yet
          birthday: undefined, // Field doesn't exist in database yet
          referredBy: undefined, // Field doesn't exist in database yet
          // Call analytics fields (don't exist in database yet)
          totalCalls: 0,
...
export async function searchCustomersFast(query: string, page: number = 1, pageSize: number = 50) {
  try {
    ...
    let searchQuery = `name.ilike.%${query}%,phone.ilike.%${query}%,email.ilike.%${query}%,city.ilike.%${query}%,referral_source.ilike.%${query}%,customer_tag.ilike.%${query}%,initial_notes.ilike.%${query}%`;
    
    // Add enhanced phone number search
    if (/^\d{3,}$/.test(query)) {
      searchQuery += `,phone.like.%${query},phone.like.${query}%`;
    }
    ...
    if (/^\d{3,}$/.test(cleanQuery)) {
      ...
      phoneVariations.forEach(phone => {
        ...
        searchQuery += `,phone.eq.${phone}`;
        searchQuery += `,phone.like.%${phone}%`;
        searchQuery += `,phone.like.${phone}%`;
      });
    }
