...
const executeSql = async (query: string, params: any[] = []): Promise<any> => {
  // Log every query being executed
  console.log('🔍 [NEON] Executing SQL:', query.substring(0, 200));
  ...
  try {
    ...
    console.log('✅ [NEON] Query successful, rows:', Array.isArray(result) ? result.length : 'N/A');
    
    // Neon returns an array of rows by default
    return result;
  } catch (error: any) {
    // ALWAYS log the full error for 400s
    console.group('❌ [NEON] SQL Error (400)');
