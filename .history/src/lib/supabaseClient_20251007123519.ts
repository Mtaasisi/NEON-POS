...
// Configure Neon for browser usage
neonConfig.fetchConnectionCache = true;

// Add fetch endpoint interceptor to log errors
const originalFetch = neonConfig.fetchEndpoint || fetch;
neonConfig.fetchEndpoint = async (url, options) => {
  try {
    const response = await originalFetch(url, options);
    
    // If we get a 400, log the details
    if (response.status === 400) {
      const clonedResponse = response.clone();
      try {
        const errorBody = await clonedResponse.text();
        console.group('❌ Neon HTTP 400 Error Details');
        ...
        console.error('Could not parse 400 error response:', e);
      }
    }
...
// Ensure databaseUrl is a string and log its type
console.log('🔍 Database URL type:', typeof databaseUrl);
console.log('🔍 Database URL value:', databaseUrl);

if (!databaseUrl || typeof databaseUrl !== 'string') {
  console.error('❌ VITE_DATABASE_URL is not defined or not a string');
  console.error('❌ Type:', typeof databaseUrl);
  console.error('❌ Value:', databaseUrl);
  throw new Error('VITE_DATABASE_URL must be a valid connection string');
}
...
const executeSql = async (query: string, params: any[] = []): Promise<any> => {
  try {
    ...
    // The neon() function returns a function that works with template literals
    const parts = [query];
    ...
    // Check if this is a common/expected error
    ...
    if (isSyntaxError || import.meta.env.DEV) {
...
class NeonQueryBuilder {
  ...
  async then(resolve?: any, reject?: any) {
    ...
    try {
      ...
      if (!isExpectedError || this.suppressErrors === false) {
        console.error(`❌ Query failed on table '${this.tableName}':`, error.message);
        if (!isExpectedError) {
          console.error('   SQL:', query.substring(0, 200));
        }
      }
  ...
  async insert(values: any) {
    try {
      ...
      const isTableNotFound = error.message?.includes('does not exist') || error.code === '42P01';
      if (!isTableNotFound) {
        console.error(`❌ Insert failed on '${this.tableName}':`, error.message || error);
      }
      return { data: null, error: { message: error.message || 'Insert failed', code: error.code || '400' } };
    }
  }
...
const rpcCall = async (fn: string, params?: any) => {
  try {
    ...
    const isFunctionNotFound = error.message?.includes('does not exist') || error.message?.includes('function') || error.code === '42883';
    if (!isFunctionNotFound) {
      console.error(`❌ RPC '${fn}' failed:`, error.message || error);
    }
    return { data: null, error: { message: error.message, code: error.code || '400' } };
  }
};
