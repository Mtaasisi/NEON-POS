...
// Disable browser warnings about SQL security (we understand the risks)
...
// Create Neon SQL client with warning suppression
const sql = neon(DATABASE_URL, {
  fetchOptions: {
    cache: 'no-store',
  },
  // Suppress the browser security warning
  fullResults: true,
});

// Suppress browser warnings by configuring before use
(sql as any).disableWarningInBrowsers = true;
...
class NeonQueryBuilder {
  ...
  select(fields: string = '*') {
    ...
    if (fields.includes('(') && fields.includes(')')) {
      ...
      // Remove all relationship patterns like "table(col1, col2)" including multi-line
      // This handles cases like "customers (\n  name,\n  phone\n)"
      fields = fields.replace(/,?\s*\w+\s*\([^)]+\)/gs, '');
