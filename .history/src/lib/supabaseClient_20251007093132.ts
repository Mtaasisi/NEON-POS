...
// Create a Pool for raw query execution
...
class NeonQueryBuilder {
  ...
  async insert(values: any) {
    try {
      ...
      // Execute the query using Neon's SQL function with proper template string array
      const templateArray = Object.assign([query], { raw: [query] });
      const data = await sql(templateArray as any);
...
const mockAuth = {
  ...
  updateUser: async (attributes: any) => {
    try {
      ...
      // Execute the query using Neon's SQL function with proper template string array
      const templateArray = Object.assign([query], { raw: [query] });
      const result = await sql(templateArray as any);
