        ...
        for (const productId of batch) {
          try {
            const { data: individualVariants, error: individualError } = await supabase
              .from('lats_product_variants')
              .select('id, product_id, variant_name, sku, cost_price, selling_price, quantity, min_quantity, created_at, updated_at')
    ...
    return (products || []).map(product => {
      ...
      return {
        ...
        variants: (variantsByProductId.get(product.id) || []).map((variant: any) => ({
          ...
          sellingPrice: variant.selling_price || variant.selling_price,
