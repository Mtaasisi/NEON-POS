...
const executeSql = async (query: string, params: any[] = [], suppressLogs: boolean = false): Promise<any> => {
  try {
    ...
    // Check if this is a known/expected error that should be handled gracefully
    ...
    // Only log unexpected errors (not table/column not found which are handled by the app)
    if (!suppressLogs && !isTableNotFound && !isColumnNotFound) {
      // In production, only log syntax errors and critical issues
