...
class NeonQueryBuilder {
  ...
  async update(values: any) {
    try {
      ...
      console.error('❌ Update failed on table:', this.tableName);
      console.error('   Error:', error.message || error);
  ...
  async delete() {
    try {
      ...
      console.error('❌ Delete failed on table:', this.tableName);
      console.error('   Error:', error.message || error);
      return { data: null, error: { message: error.message, code: error.code || '400' } };
    }
  }
  ...
  async upsert(values: any, options?: { onConflict?: string }) {
    try {
      ...
      console.error('❌ Upsert failed on table:', this.tableName);
      console.error('   Error:', error.message || error);
      return { data: null, error: { message: error.message, code: error.code || '400' } };
    }
  }
...
const rpcCall = async (fn: string, params?: any) => {
  try {
    ...
    console.error('❌ RPC call failed:', fn);
    console.error('   Error:', error.message || error);
    return { data: null, error: { message: error.message, code: error.code || '400' } };
  }
};
