...
const executeSql = async (query: string, params: any[] = []): Promise<any> => {
  try {
    ...
    console.group('❌ SQL Execution Error - DETAILED');
    ...
    console.groupEnd();
    // Throw error to be handled by caller - don't silently return empty array
    throw error;
  }
};
...
class NeonQueryBuilder {
  ...
  async insert(values: any) {
    try {
      ...
      console.error('❌ Insert failed on table:', this.tableName);
      console.error('   Error:', error.message || error);
