...
const sql = neon(databaseUrl || '', {
  ...
  arrayMode: false,  // Return objects, not arrays
});

// Helper function to execute SQL queries with the Neon serverless client
// Properly use tagged template literals with the Neon driver
const executeSql = async (query: string, params: any[] = []): Promise<any> => {
  try {
    // Neon driver requires tagged template literals
    // We need to use sql as a template tag function
    // Build the template parts properly
    const parts = [query];
    const templateParts: TemplateStringsArray = Object.assign(parts, { raw: [query] });
    
    // Call sql with the template
    const result = await sql(templateParts as any);
    return result;
  } catch (error: any) {
    ...
    // Only show detailed logging for unexpected errors
    if (!isTableNotFound) {
