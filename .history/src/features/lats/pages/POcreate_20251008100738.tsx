...
const POcreate: React.FC = () => {
  ...
  const products = useMemo(() => {
    console.log('🔍 [POcreate] Products transformation started:', {
    ...
    if (dbProducts.length === 0 || categories.length === 0) {
      console.log('⚠️ [POcreate] No products or categories loaded yet');
      return [];
    }
  ...
  const filteredProducts = useMemo(() => {
    console.log('🔍 [POcreate] Filtering products:', {
    ...
    if (showSearchResults && searchResults.length > 0) {
      console.log('✅ [POcreate] Using search results:', searchResults.length);
    ...
    console.log('✅ [POcreate] Filtered products result:', {
  ...
  useEffect(() => {
    const loadData = async () => {
      console.log('🔄 [POcreate] Starting data load...');
      try {
        ...
        console.log(`✅ [POcreate] Data loaded successfully in ${endTime - startTime}ms`);
      } catch (error) {
        console.error('❌ [POcreate] Error loading data for Purchase Order:', error);
      }
    };
  ...
  // Separate effect to log the actual data counts after state updates
  useEffect(() => {
    console.log('📊 [POcreate] Loaded data counts:', {
      products: dbProducts.length,
      categories: categories.length,
      suppliers: suppliers.length
    });
  }, [dbProducts.length, categories.length, suppliers.length]);
  ...
  useEffect(() => {
    console.log('🛒 [POcreate] Cart state changed:', {
      itemsCount: purchaseCartItems.length,
      ...
      subtotal: purchaseCartItems.reduce((sum, item) => sum + item.totalPrice, 0)
    });
  }, [purchaseCartItems]);

  // Update currency when supplier is selected
  useEffect(() => {
    console.log('👤 [POcreate] Supplier changed:', {
    ...
    if (selectedSupplier?.currency) {
      ...
      if (supplierCurrency) {
        console.log('💰 [POcreate] Updating currency to supplier currency:', selectedSupplier.currency);
  ...
  const handleAddToPurchaseCart = useCallback((product: any, variant?: any, quantity: number = 1) => {
    console.log('🛒 [POcreate] Adding to cart:', {
    ...
    if (!selectedVariant) {
      console.warn('⚠️ [POcreate] No variants available for product:', product.name);
    ...
    console.log('📦 [POcreate] Variant details:', {
    ...
    setPurchaseCartItems(prevItems => {
      ...
      if (existingItem) {
        console.log('✅ [POcreate] Updated existing cart item:', {
        ...
        console.log('✅ [POcreate] Added new cart item:', {
          itemId: newItem.id,
          name: newItem.name,
  ...
  const handleCreatePurchaseOrder = useCallback(async () => {
      console.log('🚀 [POcreate] Creating purchase order...');
      console.log('📋 [POcreate] Current state:', {
        supplierId: selectedSupplier?.id,
        supplierName: selectedSupplier?.name,
        ...
        subtotal,
        totalAmount,
        cartItems: purchaseCartItems.map(item => ({
          name: item.name,
          quantity: item.quantity,
          costPrice: item.costPrice,
          totalPrice: item.totalPrice
    ...
    if (!validation.isValid) {
      console.error('❌ [POcreate] Validation failed:', validation.errors);
      toast.error(validation.errors.join('\n'));
      return;
    }

    console.log('✅ [POcreate] Validation passed, proceeding with creation...');
    setIsCreatingPO(true);
    
    try {
      const orderNumber = generatePONumber();
      console.log('📝 [POcreate] Generated order number:', orderNumber);
      ...
      console.log('📤 [POcreate] Sending purchase order data:', purchaseOrderData);
      
      const result = await createPurchaseOrder(purchaseOrderData);
      
      console.log('📥 [POcreate] Received result:', {
        ok: result.ok,
      ...
      if (result.ok) {
        console.log('✅ [POcreate] Purchase order created successfully!', result.data);
        ...
        console.log('🧹 [POcreate] Form cleared successfully');
      } else {
        console.error('❌ [POcreate] Failed to create purchase order:', result.message);
        toast.error(result.message || 'Failed to create purchase order');
      }
    } catch (error) {
      console.error('❌ [POcreate] Error creating purchase order:', error);
