...
const PurchaseOrdersPage: React.FC = () => {
  ...
  const formatCurrency = (amount: number, currencyCode: string = 'TZS') => {
    console.log('🔍 [formatCurrency] Input:', { amount, currencyCode });
    
    if (!amount || amount === 0) return 'TSh 0';
    
    // Handle different currencies
    if (currencyCode === 'TZS' || !currencyCode) {
      const formatted = `TSh ${amount.toLocaleString('en-US', { minimumFractionDigits: 0, maximumFractionDigits: 0 })}`;
      console.log('🔍 [formatCurrency] TZS result:', formatted);
      return formatted;
    }
    ...
    try {
      ...
      const formatted = `${currencyCode} ${amount.toLocaleString('en-US', { minimumFractionDigits: 0, maximumFractionDigits: 0 })}`;
                        ...
                        <div className="flex items-center gap-2">
                          ...
                          <span className="font-bold text-gray-900">
                            {(() => {
                              ...
                              return formatCurrency(0, order.currency || 'TZS');
                            })()}
                          </span>
                        </div>
                        <div className="flex items-center gap-2">
                          ...
                          {order.exchangeRate && order.exchangeRate !== 1.0 && order.totalAmountBaseCurrency && (
                            <span className="text-xs text-blue-600">
                              {formatCurrency(order.totalAmountBaseCurrency, 'TZS')}
                            </span>
                          )}
                        </div>
                      </div>
