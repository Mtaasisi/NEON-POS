...
// Sample product image URL (using a placeholder image service)
const SAMPLE_IMAGE_URL = 'https://images.unsplash.com/photo-1505740420928-5e560c06d30e?w=800&q=80';
...
class ProductCreationTester {
  ...
  async login(username = 'admin@example.com', password = 'admin123') {
    console.log('\n🔐 Step 0: Logging in...');
  ...
  async fillProductForm(testData) {
    ...
    try {
      ...
      if (testData.name) {
        ...
        const nameInput = await this.page.locator('input[name="name"], input[placeholder*="Product name" i], input[placeholder*="Name" i]').first();
      ...
      if (testData.category) {
        ...
        try {
          const categorySelect = await this.page.locator('select[name="categoryId"], select[name="category"]').first();
          if (await categorySelect.count() > 0) {
            // Get all options
            const options = await categorySelect.locator('option').all();
            for (const option of options) {
              const text = await option.textContent();
              if (text && text.toLowerCase().includes(testData.category.toLowerCase())) {
      ...
      if (testData.condition) {
        ...
        try {
          ...
          console.log(`  ⚠️  Condition selection skipped`);
        }
      }
      ...
      if (testData.price) {
        ...
        const priceInput = await this.page.locator('input[name="price"], input[placeholder*="Price" i], input[placeholder*="Selling" i]').first();
        if (await priceInput.count() > 0) {
          await priceInput.fill(String(testData.price));
          await this.page.waitForTimeout(500);
        }
      }

      if (testData.costPrice) {
        console.log(`  📝 Filling cost price: ${testData.costPrice}`);
        const costInput = await this.page.locator('input[name="costPrice"], input[placeholder*="Cost" i]').first();
        if (await costInput.count() > 0) {
          await costInput.fill(String(testData.costPrice));
          await this.page.waitForTimeout(500);
        }
      }
      ...
      if (testData.stockQuantity !== undefined) {
        ...
        const stockInput = await this.page.locator('input[name="stockQuantity"], input[placeholder*="Stock" i], input[placeholder*="Quantity" i]').first();
        if (await stockInput.count() > 0) {
          await stockInput.fill(String(testData.stockQuantity));
          await this.page.waitForTimeout(500);
        }
      }
      ...
      if (testData.minStockLevel !== undefined) {
        ...
        const minStockInput = await this.page.locator('input[name="minStockLevel"], input[placeholder*="Minimum" i]').first();
        if (await minStockInput.count() > 0) {
          await minStockInput.fill(String(testData.minStockLevel));
          await this.page.waitForTimeout(500);
        }
      }
  ...
  async uploadProductImage(imageUrl = SAMPLE_IMAGE_URL) {
    ...
    try {
      ...
      // Ensure we have a sample image on disk
      const base64Png = 'iVBORw0KGgoAAAANSUhEUgAAAAEAAAABCAQAAAC1HAwCAAAAC0lEQVR4nGNgYAAAAAMAASsJTYQAAAAASUVORK5CYII=';
      const samplePath = join(SCREENSHOT_DIR, 'sample-image.png');
      try {
        writeFileSync(samplePath, Buffer.from(base64Png, 'base64'));
      } catch (e) {
        // ignore
      }

      // Try to find upload button or dropzone
      const uploadButton = await this.page.locator('button:has-text("Upload"), button:has-text("Add Image"), input[type="file"], [data-testid="image-upload"], [aria-label*="image" i]').first();
      
      if (await uploadButton.count() === 0) {
        console.log('  ⚠️  No image upload button found, skipping image upload');
        await this.captureScreenshot('04-no-upload-button');
      ...
      if (tagName === 'input') {
        console.log('  📁 Found file input, attempting file upload...');
        try {
          await this.page.locator('input[type="file"]').first().setInputFiles(samplePath);
          await this.page.waitForTimeout(1000);
          await this.captureScreenshot('04-file-selected');
        } catch (e) {
          console.log(`  ⚠️  setInputFiles failed: ${e.message}`);
          await this.captureScreenshot('04-file-input-found');
        }
      } else {
        console.log('  🖱️  Found upload button, clicking...');
        await uploadButton.click();
        await this.page.waitForTimeout(1000);
        await this.captureScreenshot('04-upload-clicked');
      }

      console.log('  ✅ Image upload section accessed');
      return true;
    } catch (error) {
  ...
  async runCompleteTest(testData) {
    ...
    result.steps.imageUpload = await this.uploadProductImage(testData.imageUrl);
