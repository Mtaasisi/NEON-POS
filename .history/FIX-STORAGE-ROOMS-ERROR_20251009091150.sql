...
-- Step 3: Create lats_store_shelves table with correct schema
...
BEGIN
  ...
  IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'lats_store_shelves' AND column_name = 'row_number') THEN
    ...
    RAISE NOTICE '✅ Added row_number column';
  END IF;
  ...
  IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'lats_store_shelves' AND column_name = 'column_letter') THEN
    ...
    RAISE NOTICE '✅ Added column_letter column';
  END IF;
  ...
  IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'lats_store_shelves' AND column_name = 'current_occupancy') THEN
    ...
    RAISE NOTICE '✅ Added current_occupancy column';
  END IF;
...
BEGIN
  ...
  IF room_count = 0 THEN
    ...
    INSERT INTO lats_store_shelves (storage_room_id, name, code, position, row_number, column_letter, capacity, current_occupancy, is_active)
    VALUES 
      -- Main Warehouse shelves
      (room1_id, 'Shelf A1', 'A1', 'Front Left', 1, 'A', 100, 0, true),
      (room1_id, 'Shelf A2', 'A2', 'Front Center', 1, 'B', 100, 0, true),
      (room1_id, 'Shelf B1', 'B1', 'Back Left', 2, 'A', 100, 0, true),
      (room1_id, 'Shelf B2', 'B2', 'Back Center', 2, 'B', 100, 0, true),
      -- Secure Storage shelves
      (room2_id, 'Secure A1', 'S-A1', 'Section A', 1, 'A', 50, 0, true),
      (room2_id, 'Secure B1', 'S-B1', 'Section B', 1, 'B', 50, 0, true),
      -- Display Room shelves
      (room3_id, 'Display Front', 'D-F1', 'Front Display', 1, 'A', 30, 0, true),
      (room3_id, 'Display Center', 'D-C1', 'Center Display', 1, 'B', 30, 0, true);
