 ...
 * 3. Products table images column
 * 4. Taking screenshots of the product list
 * 5. Analyzing the frontend code
 */
...
try {
  ...
  console.error('Please ensure database-config.json exists with your Neon database credentials');
  process.exit(1);
}

const supabase = createClient(dbConfig.supabaseUrl, dbConfig.supabaseServiceKey);
...
async function checkProductImagesTable() {
  console.log('📊 Step 1: Checking product_images table...');
  ...
  try {
    ...
    if (error) {
      ...
      console.log('❌ product_images table error:', error.message);
      return false;
    }
    
    console.log(`✅ product_images table exists with ${data?.length || 0} sample records`);
    ...
    DIAGNOSTIC_REPORT.databaseAnalysis.productImagesCount = count || 0;
    console.log(`   Total product images in database: ${count || 0}`);
    
    if (count === 0) {
      DIAGNOSTIC_REPORT.issues.push({
        severity: 'HIGH',
        issue: 'No images in product_images table',
        solution: 'Need to migrate images from lats_products.images column'
      });
    }
    ...
    if (data && data.length > 0) {
      ...
      DIAGNOSTIC_REPORT.databaseAnalysis.sampleProductImage = data[0];
    }
    ...
    DIAGNOSTIC_REPORT.issues.push({
      severity: 'CRITICAL',
      issue: 'Cannot query product_images table',
      error: error.message
    });
    console.log('❌ Failed to check product_images table:', error.message);
    return false;
  }
}

// 2. Check lats_products.images column
async function checkProductsImagesColumn() {
  console.log('\n📊 Step 2: Checking lats_products.images column...');
  ...
  try {
    ...
    if (error) {
      console.log('❌ Error querying lats_products:', error.message);
      return;
    }
    
    console.log(`✅ Found ${data?.length || 0} products with images column data`);
    
    // Analyze images structure
    const productsWithImages = data?.filter(p => p.images && Array.isArray(p.images) && p.images.length > 0) || [];
    const productsWithThumbnails = data?.filter(p => p.thumbnail_url) || [];
    
    DIAGNOSTIC_REPORT.databaseAnalysis.productsWithImagesColumn = productsWithImages.length;
    DIAGNOSTIC_REPORT.databaseAnalysis.productsWithThumbnailUrl = productsWithThumbnails.length;
    
    console.log(`   Products with images array: ${productsWithImages.length}`);
    console.log(`   Products with thumbnail_url: ${productsWithThumbnails.length}`);
    
    if (productsWithImages.length > 0) {
      console.log('   Sample product with images:', JSON.stringify(productsWithImages[0], null, 2));
      DIAGNOSTIC_REPORT.databaseAnalysis.sampleProductWithImages = productsWithImages[0];
    }
    ...
    const { count: productsWithImageRecords } = await supabase
      .from('product_images')
      .select('product_id', { count: 'exact', head: true });
    
    if (productsWithImages.length > productsWithImageRecords) {
      DIAGNOSTIC_REPORT.issues.push({
        severity: 'HIGH',
        issue: 'Images in lats_products.images not migrated to product_images table',
        details: `${productsWithImages.length} products have images array, but only ${productsWithImageRecords} products have records in product_images`,
        solution: 'Run migration script to populate product_images from lats_products.images'
      });
      DIAGNOSTIC_REPORT.fixes.push('Run FIX-PRODUCT-IMAGES-TABLE.sql to migrate images');
    }
    
  } catch (error) {
    console.log('❌ Error checking products images:', error.message);
  }
}

// 3. Check image URLs are valid
async function checkImageURLs() {
  console.log('\n📊 Step 3: Checking image URL validity...');
  ...
  try {
    ...
    if (!images || images.length === 0) {
      console.log('⚠️  No images to check');
      return;
    }
    ...
    DIAGNOSTIC_REPORT.databaseAnalysis.validImageUrls = validUrls;
    DIAGNOSTIC_REPORT.databaseAnalysis.invalidImageUrls = invalidUrls;
    
  } catch (error) {
    console.log('❌ Error checking URLs:', error.message);
  }
}

// 4. Take screenshots of product pages
async function takeProductScreenshots() {
  console.log('\n📸 Step 4: Taking screenshots of product pages...');
