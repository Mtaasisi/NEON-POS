-- ============================================
-- COMPLETE FIX FOR SETTINGS 400 ERRORS
-- This script does everything in one go:
-- 1. Cleans up duplicate settings
-- 2. Drops old incorrectly named tables
-- 3. Creates new tables with correct lats_pos_ prefix
-- 4. Disables RLS and grants permissions
-- ============================================

SELECT '🔧 Starting complete settings fix...' as status;

-- ============================================
-- STEP 1: Clean up duplicate settings in NEW tables (if they exist)
-- ============================================

SELECT '📋 Step 1: Cleaning up duplicate settings...' as status;
  ...
  LOOP
    ...
    IF table_exists THEN
      ...
      IF deleted_count > 0 THEN
        RAISE NOTICE '✅ Deleted % duplicate records from %', deleted_count, table_record.table_name;
      ELSE
        RAISE NOTICE '✅ No duplicates in %', table_record.table_name;
      END IF;
    ELSE
      RAISE NOTICE '⏭️  Table % does not exist yet', table_record.table_name;
    END IF;
  END LOOP;
END $$;

-- ============================================
-- STEP 2: Drop old incorrectly named tables
-- ============================================

SELECT '🗑️  Step 2: Dropping old incorrectly named tables...' as status;
...
-- ============================================
-- STEP 3: Create tables with correct lats_pos_ prefix
-- ============================================

SELECT '🏗️  Step 3: Creating tables with correct names...' as status;
...
CREATE TABLE IF NOT EXISTS lats_pos_general_settings (
  ...
  CONSTRAINT lats_pos_general_settings_user_id_unique UNIQUE (user_id)
);
...
CREATE TABLE IF NOT EXISTS lats_pos_dynamic_pricing_settings (
  ...
  created_at TIMESTAMP WITH TIME ZONE DEFAULT NOW(),
  updated_at TIMESTAMP WITH TIME ZONE DEFAULT NOW(),
  CONSTRAINT lats_pos_dynamic_pricing_settings_user_id_unique UNIQUE (user_id)
);
...
CREATE TABLE IF NOT EXISTS lats_pos_receipt_settings (
  ...
  created_at TIMESTAMP WITH TIME ZONE DEFAULT NOW(),
  updated_at TIMESTAMP WITH TIME ZONE DEFAULT NOW(),
  CONSTRAINT lats_pos_receipt_settings_user_id_unique UNIQUE (user_id)
);
...
CREATE TABLE IF NOT EXISTS lats_pos_barcode_scanner_settings (
  ...
  created_at TIMESTAMP WITH TIME ZONE DEFAULT NOW(),
  updated_at TIMESTAMP WITH TIME ZONE DEFAULT NOW(),
  CONSTRAINT lats_pos_barcode_scanner_settings_user_id_unique UNIQUE (user_id)
);
...
CREATE TABLE IF NOT EXISTS lats_pos_delivery_settings (
  ...
  created_at TIMESTAMP WITH TIME ZONE DEFAULT NOW(),
  updated_at TIMESTAMP WITH TIME ZONE DEFAULT NOW(),
  CONSTRAINT lats_pos_delivery_settings_user_id_unique UNIQUE (user_id)
);
...
CREATE TABLE IF NOT EXISTS lats_pos_search_filter_settings (
  ...
  created_at TIMESTAMP WITH TIME ZONE DEFAULT NOW(),
  updated_at TIMESTAMP WITH TIME ZONE DEFAULT NOW(),
  CONSTRAINT lats_pos_search_filter_settings_user_id_unique UNIQUE (user_id)
);
...
CREATE TABLE IF NOT EXISTS lats_pos_user_permissions_settings (
  ...
  created_at TIMESTAMP WITH TIME ZONE DEFAULT NOW(),
  updated_at TIMESTAMP WITH TIME ZONE DEFAULT NOW(),
  CONSTRAINT lats_pos_user_permissions_settings_user_id_unique UNIQUE (user_id)
);
...
CREATE TABLE IF NOT EXISTS lats_pos_loyalty_customer_settings (
  ...
  created_at TIMESTAMP WITH TIME ZONE DEFAULT NOW(),
  updated_at TIMESTAMP WITH TIME ZONE DEFAULT NOW(),
  CONSTRAINT lats_pos_loyalty_customer_settings_user_id_unique UNIQUE (user_id)
);
...
CREATE TABLE IF NOT EXISTS lats_pos_analytics_reporting_settings (
  ...
  created_at TIMESTAMP WITH TIME ZONE DEFAULT NOW(),
  updated_at TIMESTAMP WITH TIME ZONE DEFAULT NOW(),
  CONSTRAINT lats_pos_analytics_reporting_settings_user_id_unique UNIQUE (user_id)
);
...
CREATE TABLE IF NOT EXISTS lats_pos_notification_settings (
  ...
  created_at TIMESTAMP WITH TIME ZONE DEFAULT NOW(),
  updated_at TIMESTAMP WITH TIME ZONE DEFAULT NOW(),
  CONSTRAINT lats_pos_notification_settings_user_id_unique UNIQUE (user_id)
);
...
CREATE TABLE IF NOT EXISTS lats_pos_advanced_settings (
  ...
  created_at TIMESTAMP WITH TIME ZONE DEFAULT NOW(),
  updated_at TIMESTAMP WITH TIME ZONE DEFAULT NOW(),
  CONSTRAINT lats_pos_advanced_settings_user_id_unique UNIQUE (user_id)
);

SELECT '✅ All tables created with correct names' as status;

-- ============================================
-- STEP 4: Disable RLS and grant permissions
-- ============================================

SELECT '🔒 Step 4: Disabling RLS and granting permissions...' as status;
...
-- Grant permissions (Neon doesn't have postgres role, only anon, authenticated, service_role)
GRANT ALL ON lats_pos_general_settings TO anon, authenticated, service_role;
GRANT ALL ON lats_pos_dynamic_pricing_settings TO anon, authenticated, service_role;
GRANT ALL ON lats_pos_receipt_settings TO anon, authenticated, service_role;
GRANT ALL ON lats_pos_barcode_scanner_settings TO anon, authenticated, service_role;
GRANT ALL ON lats_pos_delivery_settings TO anon, authenticated, service_role;
GRANT ALL ON lats_pos_search_filter_settings TO anon, authenticated, service_role;
GRANT ALL ON lats_pos_user_permissions_settings TO anon, authenticated, service_role;
GRANT ALL ON lats_pos_loyalty_customer_settings TO anon, authenticated, service_role;
GRANT ALL ON lats_pos_analytics_reporting_settings TO anon, authenticated, service_role;
GRANT ALL ON lats_pos_notification_settings TO anon, authenticated, service_role;
GRANT ALL ON lats_pos_advanced_settings TO anon, authenticated, service_role;
...
SELECT '📇 Step 5: Creating indexes...' as status;
...
SELECT '✅ Step 6: Verifying the fix...' as status;
...
BEGIN
  ...
  LOOP
    ...
    IF duplicate_count > 0 THEN
      RAISE NOTICE '⚠️  Table % has duplicates!', table_record.table_name;
    ELSE
      RAISE NOTICE '✅ Table % OK (% records, no duplicates)', table_record.table_name, record_count;
    END IF;
  END LOOP;
END $$;
...
SELECT '🎉 COMPLETE SETTINGS FIX DONE!' as result;
SELECT 'All settings tables created with correct lats_pos_ prefix' as info_1;
SELECT 'Duplicates cleaned up, RLS disabled, permissions granted' as info_2;
SELECT 'Now refresh your app (Ctrl+Shift+R or Cmd+Shift+R)' as next_step;
SELECT 'The 400 errors and duplicate warnings should be GONE! ✅' as final_message;
