...
BEGIN
  ...
  -- Create finance transaction if finance_transactions table exists (using dynamic SQL to avoid parse errors)
  BEGIN
    IF EXISTS (
      SELECT 1 FROM information_schema.tables 
      WHERE table_schema = 'public' AND table_name = 'finance_transactions'
    ) THEN
      -- Use dynamic SQL to avoid function validation errors if table doesn't exist
      EXECUTE format(
