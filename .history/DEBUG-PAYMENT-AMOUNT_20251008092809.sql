...
-- 3. Check if products exist and have prices
SELECT 
  '📦 PRODUCTS WITH PRICES' as debug_type,
  COUNT(*) as total_products,
  COUNT(CASE WHEN selling_price > 0 THEN 1 END) as products_with_price,
  AVG(selling_price) as avg_price
FROM lats_products;

-- 4. Check product variants (where prices are usually stored)
SELECT 
  '🏷️ PRODUCT VARIANTS' as debug_type,
  COUNT(*) as total_variants,
  COUNT(CASE WHEN selling_price > 0 THEN 1 END) as variants_with_price,
  AVG(selling_price) as avg_variant_price
...
WHERE p.selling_price > 0 OR v.selling_price > 0
