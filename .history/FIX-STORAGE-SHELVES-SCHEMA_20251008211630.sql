...
EXCEPTION WHEN OTHERS THEN
  RAISE NOTICE 'Note: Some tables may not exist yet: %', SQLERRM;
END $$;
...
BEGIN
  ...
  IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name = 'lats_store_locations') THEN
    GRANT ALL ON lats_store_locations TO PUBLIC;
  END IF;
  
  RAISE NOTICE '✅ Granted permissions on existing tables';
EXCEPTION WHEN OTHERS THEN
  RAISE NOTICE 'Note: Could not grant permissions: %', SQLERRM;
END $$;
