...
class ProductCreationTester {
  ...
  async login(username = 'admin', password = 'admin123') {
    ...
    try {
      ...
      // Fill login form
      const usernameInput = await this.page.locator('input[name="username"], input[type="text"], input[placeholder*="Username" i]').first();
  ...
  async navigateToProductCreation() {
    ...
    try {
      await this.page.goto(`${BASE_URL}/lats/products/new`, {
        waitUntil: 'networkidle',
        timeout: 30000
      });
  ...
  async uploadProductImage(imageUrl = SAMPLE_IMAGE_URL) {
    ...
    try {
      ...
      const uploadButton = await this.page.locator('button:has-text("Upload"), button:has-text("Add Image"), input[type="file"]').first();
      ...
      if (tagName === 'input') {
        ...
        // For demo purposes, we'll skip actual file upload and just note it
        console.log('  ℹ️  File upload would happen here (skipped in demo)');
