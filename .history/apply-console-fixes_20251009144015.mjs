...
async function createDailySalesClosuresTable() {
  ...
  try {
    ...
    // Create indexes
    await sql.unsafe('CREATE INDEX IF NOT EXISTS idx_daily_sales_closures_date ON daily_sales_closures(date DESC)');
    await sql.unsafe('CREATE INDEX IF NOT EXISTS idx_daily_sales_closures_closed_at ON daily_sales_closures(closed_at DESC)');
    await sql.unsafe('CREATE INDEX IF NOT EXISTS idx_daily_sales_closures_closed_by_user_id ON daily_sales_closures(closed_by_user_id)');
    
    // Enable RLS
    await sql.unsafe('ALTER TABLE daily_sales_closures ENABLE ROW LEVEL SECURITY');
    ...
    // Create trigger
    await sql.unsafe('DROP TRIGGER IF EXISTS update_daily_sales_closures_updated_at_trigger ON daily_sales_closures');
    await sql.unsafe(`
...
async function createPOSSettingsTables() {
  ...
  for (const table of tables) {
    try {
      // Execute table creation SQL directly as string
      await sql.unsafe(table.sql);
      
      // Create indexes using string templates
      const indexUserIdName = `idx_${table.name}_user_id`;
      const indexBusinessIdName = `idx_${table.name}_business_id`;
      
      await sql.unsafe(`CREATE INDEX IF NOT EXISTS ${indexUserIdName} ON ${table.name}(user_id)`);
      await sql.unsafe(`CREATE INDEX IF NOT EXISTS ${indexBusinessIdName} ON ${table.name}(business_id)`);
      
      // Enable RLS
      await sql.unsafe(`ALTER TABLE ${table.name} ENABLE ROW LEVEL SECURITY`);
