-- =============================================================================
-- FIX MISSING TABLES AND SETTINGS (NEON DATABASE)
-- =============================================================================
-- This script creates all missing POS settings tables and the daily_sales_closures table
-- Run this script to fix all the errors shown in the console
-- =============================================================================

-- ========================================
-- 1. CREATE DAILY SALES CLOSURES TABLE
-- ========================================
...
-- ========================================
-- 2. CREATE POS SETTINGS TABLES
-- ========================================

-- 2.1 General Settings Table
...
-- 2.2 Dynamic Pricing Settings Table
...
-- 2.4 Barcode Scanner Settings Table
...
-- 2.6 Search Filter Settings Table
...
-- 2.7 User Permissions Settings Table
...
-- 2.8 Loyalty Customer Settings Table
...
-- 2.9 Analytics Reporting Settings Table
...
-- 2.10 Notification Settings Table
...
CREATE INDEX IF NOT EXISTS idx_lats_pos_general_settings_business_id ON lats_pos_general_settings(business_id);

CREATE INDEX IF NOT EXISTS idx_lats_pos_dynamic_pricing_settings_user_id ON lats_pos_dynamic_pricing_settings(user_id);
CREATE INDEX IF NOT EXISTS idx_lats_pos_dynamic_pricing_settings_business_id ON lats_pos_dynamic_pricing_settings(business_id);

CREATE INDEX IF NOT EXISTS idx_lats_pos_receipt_settings_user_id ON lats_pos_receipt_settings(user_id);
CREATE INDEX IF NOT EXISTS idx_lats_pos_receipt_settings_business_id ON lats_pos_receipt_settings(business_id);

CREATE INDEX IF NOT EXISTS idx_lats_pos_barcode_scanner_settings_user_id ON lats_pos_barcode_scanner_settings(user_id);
CREATE INDEX IF NOT EXISTS idx_lats_pos_barcode_scanner_settings_business_id ON lats_pos_barcode_scanner_settings(business_id);

CREATE INDEX IF NOT EXISTS idx_lats_pos_delivery_settings_user_id ON lats_pos_delivery_settings(user_id);
CREATE INDEX IF NOT EXISTS idx_lats_pos_delivery_settings_business_id ON lats_pos_delivery_settings(business_id);

CREATE INDEX IF NOT EXISTS idx_lats_pos_search_filter_settings_user_id ON lats_pos_search_filter_settings(user_id);
CREATE INDEX IF NOT EXISTS idx_lats_pos_search_filter_settings_business_id ON lats_pos_search_filter_settings(business_id);

CREATE INDEX IF NOT EXISTS idx_lats_pos_user_permissions_settings_user_id ON lats_pos_user_permissions_settings(user_id);
CREATE INDEX IF NOT EXISTS idx_lats_pos_user_permissions_settings_business_id ON lats_pos_user_permissions_settings(business_id);

CREATE INDEX IF NOT EXISTS idx_lats_pos_loyalty_customer_settings_user_id ON lats_pos_loyalty_customer_settings(user_id);
CREATE INDEX IF NOT EXISTS idx_lats_pos_loyalty_customer_settings_business_id ON lats_pos_loyalty_customer_settings(business_id);
...
CREATE INDEX IF NOT EXISTS idx_lats_pos_analytics_reporting_settings_business_id ON lats_pos_analytics_reporting_settings(business_id);

CREATE INDEX IF NOT EXISTS idx_lats_pos_notification_settings_user_id ON lats_pos_notification_settings(user_id);
CREATE INDEX IF NOT EXISTS idx_lats_pos_notification_settings_business_id ON lats_pos_notification_settings(business_id);

CREATE INDEX IF NOT EXISTS idx_lats_pos_advanced_settings_user_id ON lats_pos_advanced_settings(user_id);
CREATE INDEX IF NOT EXISTS idx_lats_pos_advanced_settings_business_id ON lats_pos_advanced_settings(business_id);

-- ========================================
-- 4. ENABLE RLS ON ALL SETTINGS TABLES
-- ========================================
...
-- ========================================
-- 5. CREATE RLS POLICIES FOR ALL SETTINGS TABLES
-- ========================================
...
BEGIN
    IF NOT EXISTS (
        SELECT 1 FROM pg_policies 
        WHERE schemaname = 'public' 
        AND tablename = 'lats_pos_general_settings' 
        AND policyname = 'Allow all operations on general settings'
    ) THEN
        CREATE POLICY "Allow all operations on general settings"
            ON lats_pos_general_settings
            FOR ALL
            USING (true)
            WITH CHECK (true);
    END IF;
...
BEGIN
    IF NOT EXISTS (
        SELECT 1 FROM pg_policies 
        WHERE schemaname = 'public' 
        AND tablename = 'lats_pos_dynamic_pricing_settings' 
        AND policyname = 'Allow all operations on dynamic pricing settings'
    ) THEN
        CREATE POLICY "Allow all operations on dynamic pricing settings"
            ON lats_pos_dynamic_pricing_settings
            FOR ALL
            USING (true)
            WITH CHECK (true);
    END IF;
...
BEGIN
    IF NOT EXISTS (
        SELECT 1 FROM pg_policies 
        WHERE schemaname = 'public' 
        AND tablename = 'lats_pos_receipt_settings' 
        AND policyname = 'Allow all operations on receipt settings'
    ) THEN
        CREATE POLICY "Allow all operations on receipt settings"
            ON lats_pos_receipt_settings
            FOR ALL
            USING (true)
            WITH CHECK (true);
    END IF;
...
BEGIN
    IF NOT EXISTS (
        SELECT 1 FROM pg_policies 
        WHERE schemaname = 'public' 
        AND tablename = 'lats_pos_barcode_scanner_settings' 
        AND policyname = 'Allow all operations on barcode scanner settings'
    ) THEN
        CREATE POLICY "Allow all operations on barcode scanner settings"
            ON lats_pos_barcode_scanner_settings
            FOR ALL
            USING (true)
            WITH CHECK (true);
    END IF;
...
BEGIN
    IF NOT EXISTS (
        SELECT 1 FROM pg_policies 
        WHERE schemaname = 'public' 
        AND tablename = 'lats_pos_delivery_settings' 
        AND policyname = 'Allow all operations on delivery settings'
    ) THEN
        CREATE POLICY "Allow all operations on delivery settings"
            ON lats_pos_delivery_settings
            FOR ALL
            USING (true)
            WITH CHECK (true);
    END IF;
...
BEGIN
    IF NOT EXISTS (
        SELECT 1 FROM pg_policies 
        WHERE schemaname = 'public' 
        AND tablename = 'lats_pos_search_filter_settings' 
        AND policyname = 'Allow all operations on search filter settings'
    ) THEN
        CREATE POLICY "Allow all operations on search filter settings"
            ON lats_pos_search_filter_settings
            FOR ALL
            USING (true)
            WITH CHECK (true);
    END IF;
...
BEGIN
    IF NOT EXISTS (
        SELECT 1 FROM pg_policies 
        WHERE schemaname = 'public' 
        AND tablename = 'lats_pos_user_permissions_settings' 
        AND policyname = 'Allow all operations on user permissions settings'
    ) THEN
        CREATE POLICY "Allow all operations on user permissions settings"
            ON lats_pos_user_permissions_settings
            FOR ALL
            USING (true)
            WITH CHECK (true);
    END IF;
...
BEGIN
    IF NOT EXISTS (
        SELECT 1 FROM pg_policies 
        WHERE schemaname = 'public' 
        AND tablename = 'lats_pos_loyalty_customer_settings' 
        AND policyname = 'Allow all operations on loyalty customer settings'
    ) THEN
        CREATE POLICY "Allow all operations on loyalty customer settings"
            ON lats_pos_loyalty_customer_settings
            FOR ALL
            USING (true)
            WITH CHECK (true);
    END IF;
...
BEGIN
    IF NOT EXISTS (
        SELECT 1 FROM pg_policies 
        WHERE schemaname = 'public' 
        AND tablename = 'lats_pos_analytics_reporting_settings' 
        AND policyname = 'Allow all operations on analytics reporting settings'
    ) THEN
        CREATE POLICY "Allow all operations on analytics reporting settings"
            ON lats_pos_analytics_reporting_settings
            FOR ALL
            USING (true)
            WITH CHECK (true);
    END IF;
...
BEGIN
    IF NOT EXISTS (
        SELECT 1 FROM pg_policies 
        WHERE schemaname = 'public' 
        AND tablename = 'lats_pos_notification_settings' 
        AND policyname = 'Allow all operations on notification settings'
    ) THEN
        CREATE POLICY "Allow all operations on notification settings"
            ON lats_pos_notification_settings
            FOR ALL
            USING (true)
            WITH CHECK (true);
    END IF;
...
BEGIN
    IF NOT EXISTS (
        SELECT 1 FROM pg_policies 
        WHERE schemaname = 'public' 
        AND tablename = 'lats_pos_advanced_settings' 
        AND policyname = 'Allow all operations on advanced settings'
    ) THEN
        CREATE POLICY "Allow all operations on advanced settings"
            ON lats_pos_advanced_settings
            FOR ALL
            USING (true)
            WITH CHECK (true);
    END IF;
...
BEGIN
    ...
    RAISE NOTICE '   - daily_sales_closures';
    RAISE NOTICE '   - lats_pos_general_settings';
    RAISE NOTICE '   - lats_pos_dynamic_pricing_settings';
    RAISE NOTICE '   - lats_pos_receipt_settings';
    RAISE NOTICE '   - lats_pos_barcode_scanner_settings';
    RAISE NOTICE '   - lats_pos_delivery_settings';
    RAISE NOTICE '   - lats_pos_search_filter_settings';
    RAISE NOTICE '   - lats_pos_user_permissions_settings';
    RAISE NOTICE '   - lats_pos_loyalty_customer_settings';
    RAISE NOTICE '   - lats_pos_analytics_reporting_settings';
    RAISE NOTICE '   - lats_pos_notification_settings';
    RAISE NOTICE '   - lats_pos_advanced_settings';
    RAISE NOTICE '🔒 RLS policies enabled on all tables';
    RAISE NOTICE '📈 Indexes created for optimal performance';
    RAISE NOTICE '💡 Note: All tables use permissive policies for Neon database';
    RAISE NOTICE '';
    RAISE NOTICE '🎉 You can now reload your application - all errors should be fixed!';
END $$;
