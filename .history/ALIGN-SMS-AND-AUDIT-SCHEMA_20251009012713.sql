...
-- 12) Compatibility views for legacy vs new columns
DO $$
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM information_schema.views WHERE table_schema = 'public' AND table_name = 'v_sms_logs'
  ) THEN
    EXECUTE $$
...
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM information_schema.views WHERE table_schema = 'public' AND table_name = 'v_sms_triggers'
  ) THEN
    EXECUTE $$
...
-- 14) Guarded CHECK constraint for devices.status
-- Only add if no invalid statuses present
DO $$
