  ...
  async fillProductForm(testData) {
    ...
    try {
      ...
      if (testData.name) {
        ...
        const nameInput = await this.page.locator('#product-name, input[name="name"], input[placeholder*="Product name" i]').first();
        await nameInput.click({ timeout: 10000 });
        await nameInput.fill('');
        await nameInput.type(testData.name, { delay: 20 });
