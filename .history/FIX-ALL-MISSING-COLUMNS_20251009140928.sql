...
-- 15. Fix lats_products table - add tags column (JSONB)
DO $$ 
BEGIN
    IF NOT EXISTS (
        SELECT 1 FROM information_schema.columns 
        ...
        COMMENT ON COLUMN lats_products.tags IS 'Product tags for filtering and search (JSONB array)';
        RAISE NOTICE '✅ Added tags to lats_products';
    ELSE
        -- If column exists but is TEXT[], convert it to JSONB
        IF EXISTS (
            SELECT 1 FROM information_schema.columns 
            WHERE table_name = 'lats_products' 
            AND column_name = 'tags'
            ...
            ALTER TABLE lats_products ALTER COLUMN tags DROP DEFAULT;
            ALTER TABLE lats_products ALTER COLUMN tags TYPE JSONB 
            USING CASE 
                WHEN tags IS NULL THEN '[]'::jsonb
                WHEN tags = '{}' THEN '[]'::jsonb
                ELSE array_to_json(tags)::jsonb
            END;
            ALTER TABLE lats_products ALTER COLUMN tags SET DEFAULT '[]'::jsonb;
            RAISE NOTICE '✅ Converted tags from TEXT[] to JSONB';
        ELSE
...
-- 18. Fix lats_product_variants table - add name column
DO $$ 
BEGIN
    IF NOT EXISTS (
        SELECT 1 FROM information_schema.columns 
        ...
        COMMENT ON COLUMN lats_product_variants.name IS 'Variant display name';
        RAISE NOTICE '✅ Added name to lats_product_variants';
    ELSE
        RAISE NOTICE '✓ lats_product_variants.name already exists';
    END IF;
END $$;

-- 19. Fix lats_product_variants table - add selling_price column
DO $$ 
BEGIN
    IF NOT EXISTS (
        SELECT 1 FROM information_schema.columns 
        ...
        COMMENT ON COLUMN lats_product_variants.selling_price IS 'Variant selling price';
        RAISE NOTICE '✅ Added selling_price to lats_product_variants';
    ELSE
        RAISE NOTICE '✓ lats_product_variants.selling_price already exists';
    END IF;
END $$;

-- 20. Fix lats_product_variants table - add attributes column
DO $$ 
BEGIN
    IF NOT EXISTS (
        SELECT 1 FROM information_schema.columns 
        ...
        COMMENT ON COLUMN lats_product_variants.attributes IS 'Variant attributes (color, size, etc.)';
        RAISE NOTICE '✅ Added attributes to lats_product_variants';
    ELSE
        RAISE NOTICE '✓ lats_product_variants.attributes already exists';
    END IF;
END $$;

-- 21. Fix lats_product_variants table - add weight column
DO $$ 
BEGIN
    IF NOT EXISTS (
        SELECT 1 FROM information_schema.columns 
        ...
        COMMENT ON COLUMN lats_product_variants.weight IS 'Variant weight';
        RAISE NOTICE '✅ Added weight to lats_product_variants';
    ELSE
        RAISE NOTICE '✓ lats_product_variants.weight already exists';
    END IF;
END $$;

-- 22. Fix lats_product_variants table - add dimensions column
DO $$ 
BEGIN
    IF NOT EXISTS (
        SELECT 1 FROM information_schema.columns 
        ...
        COMMENT ON COLUMN lats_product_variants.dimensions IS 'Variant dimensions (width, height, length)';
        RAISE NOTICE '✅ Added dimensions to lats_product_variants';
    ELSE
        RAISE NOTICE '✓ lats_product_variants.dimensions already exists';
    END IF;
END $$;

-- 23. Fix lats_product_variants table - make variant_name nullable
DO $$ 
BEGIN
    -- Check if variant_name has NOT NULL constraint
    IF EXISTS (
        SELECT 1 FROM information_schema.columns 
        WHERE table_name = 'lats_product_variants' 
        ...
        UPDATE lats_product_variants SET variant_name = 'Default' WHERE variant_name IS NULL;
        
        RAISE NOTICE '✅ Made variant_name nullable with default value';
    ELSE
        RAISE NOTICE '✓ lats_product_variants.variant_name is already nullable';
    END IF;
END $$;
...
SELECT 
    ...
    ) THEN '✅ tags exists' ELSE '❌ tags missing' END
UNION ALL
SELECT 
    '================================================',
    'lats_products',
    CASE WHEN EXISTS (
        SELECT 1 FROM information_schema.columns 
        WHERE table_name = 'lats_products' AND column_name = 'attributes'
    ) THEN '✅ attributes exists' ELSE '❌ attributes missing' END
UNION ALL
SELECT 
    '================================================',
    'lats_products',
    CASE WHEN EXISTS (
        SELECT 1 FROM information_schema.columns 
        WHERE table_name = 'lats_products' AND column_name = 'metadata'
...
SELECT 
    ...
    ) THEN '✅ name exists' ELSE '❌ name missing' END
UNION ALL
SELECT 
    '================================================',
    'lats_product_variants',
...
SELECT 
    ...
    ) THEN '✅ weight exists' ELSE '❌ weight missing' END
UNION ALL
SELECT 
    '================================================',
    'lats_product_variants',
    ...
    ) THEN '✅ dimensions exists' ELSE '❌ dimensions missing' END;
...
- tags (JSONB), attributes (JSONB), metadata (JSONB)

VARIANTS TABLE (lats_product_variants):
- name, selling_price
- attributes (JSONB)
- weight, dimensions (JSONB)
- variant_name (now nullable)
...
✅ Product creation with variants is fully functional!
================================================
