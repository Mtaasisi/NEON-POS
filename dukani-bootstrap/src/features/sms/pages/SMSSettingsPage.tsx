...
const SMSSettingsPage: React.FC = () => {
  ...
  const fetchSMSSettings = async () => {
    try {
      ...
      console.log('📥 SMS settings response:', { data, error });

      if (error) {
        // Silently handle the error if it's a missing table/permission issue
        if (error.code === 'PGRST116' || error.message?.includes('does not exist')) {
          console.warn('⚠️ SMS settings table not configured yet');
        } else {
          console.warn('❌ SMS settings database error:', error);
          toast.error('Failed to fetch SMS settings');
        }
        return;
      }

      console.log('✅ Successfully fetched SMS settings:', data);
      setSettings(data || []);
      
      // Populate form data
      const apiKeySetting = data?.find(s => s.key === 'sms_provider_api_key');
      const apiUrlSetting = data?.find(s => s.key === 'sms_api_url');
      const passwordSetting = data?.find(s => s.key === 'sms_provider_password');

      console.log('🔧 Found settings:', { apiKeySetting, apiUrlSetting, passwordSetting });

      setFormData({
        apiKey: apiKeySetting?.value || '',
        apiUrl: apiUrlSetting?.value || '',
        price: '15' // Default price since not in DB yet
      });
      ...
      console.log('🚨 Exception caught in fetchSMSSettings:', err);
      ...
      if (err?.error && err?.data !== undefined) {
        ...
        if (error.code === 'PGRST116' || error.message?.includes('does not exist')) {
          ...
          console.warn('❌ SMS settings database error (from thrown response):', error);
          toast.error('Failed to fetch SMS settings');
        }
      } else {
        console.warn('❓ Unable to load SMS settings - unexpected error:', err);
      }
    } finally {
      setLoading(false);
    }
