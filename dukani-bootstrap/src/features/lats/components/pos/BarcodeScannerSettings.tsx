...
import { Scan, Settings, Save, RefreshCw, TestTube, AlertCircle, CheckCircle } from 'lucide-react';
...
const BarcodeScannerSettings: React.FC = () => {
  ...
  const loadCurrentSettings = async () => {
    setIsLoading(true);
    try {
      const settings = await POSSettingsService.loadBarcodeScannerSettings();
      if (settings) {
        // Map database fields to form fields
        reset({
          enableScanner: settings.enable_barcode_scanner ?? true,
          autoScan: settings.auto_add_to_cart ?? true,
          scanTimeout: settings.scanner_timeout ?? 5000,
          scanRetries: settings.max_retry_attempts ?? 3,
          autoAddToCart: settings.auto_add_to_cart ?? true,
          playBeepSound: settings.play_sound_on_scan ?? true,
          vibrateOnScan: settings.vibrate_on_scan ?? false,
          showScanHistory: settings.enable_scan_history ?? true,
          maxHistoryItems: settings.max_scan_history ?? 20,
          showScanErrors: settings.show_invalid_barcode_alert ?? true,
          retryOnError: settings.retry_on_error ?? true,
          errorTimeout: 3000,
          scannerType: settings.scanner_connection_type ?? 'usb',
          deviceName: settings.scanner_device_name ?? 'Default Scanner',
          connectionTimeout: settings.scanner_timeout ?? 10000,
          autoReconnect: true,
          supportedCodes: {
            ean13: settings.support_ean13 ?? true,
            ean8: settings.support_ean8 ?? true,
            upc: settings.support_upc_a ?? true,
            code128: settings.support_code128 ?? true,
            code39: settings.support_code39 ?? true,
            qr: settings.support_qr_code ?? false,
            datamatrix: settings.support_data_matrix ?? false
          },
          scanDelay: settings.scan_delay ?? 100,
          debounceTime: 300,
          caseSensitive: false,
          trimWhitespace: true
        });
  ...
  const handleSaveSettings = async (data: BarcodeScannerSettings) => {
    ...
    try {
      ...
      const dbSettings = {
        enable_barcode_scanner: data.enableScanner,
        enable_camera_scanner: false,
        enable_keyboard_input: true,
        enable_manual_entry: true,
        auto_add_to_cart: data.autoAddToCart,
        auto_focus_search: true,
        play_sound_on_scan: data.playBeepSound,
        vibrate_on_scan: data.vibrateOnScan,
        show_scan_feedback: true,
        show_invalid_barcode_alert: data.showScanErrors,
        allow_unknown_products: false,
        prompt_for_unknown_products: true,
        retry_on_error: data.retryOnError,
        max_retry_attempts: data.scanRetries,
        scanner_device_name: data.deviceName,
        scanner_connection_type: data.scannerType as 'usb' | 'bluetooth' | 'wifi',
        scanner_timeout: data.scanTimeout,
        support_ean13: data.supportedCodes.ean13,
        support_ean8: data.supportedCodes.ean8,
        support_upc_a: data.supportedCodes.upc,
        support_upc_e: data.supportedCodes.upc,
        support_code128: data.supportedCodes.code128,
        support_code39: data.supportedCodes.code39,
        support_qr_code: data.supportedCodes.qr,
        support_data_matrix: data.supportedCodes.datamatrix,
        enable_continuous_scanning: false,
        scan_delay: data.scanDelay,
        enable_scan_history: data.showScanHistory,
        max_scan_history: data.maxHistoryItems
      };

      await POSSettingsService.saveBarcodeScannerSettings(dbSettings);
