...
const DeliverySettings: React.FC = () => {
  ...
  const loadCurrentSettings = async () => {
    setIsLoading(true);
    try {
      const settings = await POSSettingsService.loadDeliverySettings();
      if (settings) {
        // Map database fields to form fields
        reset({
          enableDelivery: settings.enable_delivery ?? true,
          defaultDeliveryFee: settings.default_delivery_fee ?? 0,
          freeDeliveryThreshold: settings.free_delivery_threshold ?? 0,
          maxDeliveryDistance: settings.max_delivery_distance ?? 20,
          enableDeliveryAreas: settings.enable_delivery_areas ?? false,
          deliveryAreas: [],
          deliveryHoursEnabled: settings.enable_delivery_hours ?? false,
          deliveryStartTime: settings.delivery_start_time ?? '08:00',
          deliveryEndTime: settings.delivery_end_time ?? '18:00',
          sameDayDelivery: settings.enable_same_day_delivery ?? true,
          nextDayDelivery: settings.enable_next_day_delivery ?? true,
          deliveryTimeSlots: [
            { id: '1', time: '09:00 - 12:00', enabled: true },
            { id: '2', time: '12:00 - 15:00', enabled: true },
            { id: '3', time: '15:00 - 18:00', enabled: true },
          ...
          notifyCustomer: settings.notify_customer_on_delivery ?? true,
          notifyDriver: settings.notify_driver_on_assignment ?? true,
          sendSMS: settings.enable_sms_notifications ?? true,
          sendEmail: settings.enable_email_notifications ?? false,
          autoAssignDriver: settings.enable_driver_assignment ?? true,
          driverCommission: settings.driver_commission ?? 10,
          requireSignature: settings.require_signature ?? true,
          allowScheduledDelivery: settings.enable_scheduled_delivery ?? false,
          maxScheduledDays: 7,
          requireAdvancePayment: settings.require_advance_payment ?? false,
          allowPartialDelivery: settings.enable_partial_delivery ?? false
        });
      }
    } catch (error) {
  ...
  const handleSaveSettings = async (data: DeliverySettings) => {
    ...
    try {
      ...
      const dbSettings = {
        enable_delivery: data.enableDelivery,
        default_delivery_fee: data.defaultDeliveryFee,
        free_delivery_threshold: data.freeDeliveryThreshold,
        max_delivery_distance: data.maxDeliveryDistance,
        enable_delivery_areas: data.enableDeliveryAreas,
        delivery_areas: data.deliveryAreas || [],
        area_delivery_fees: {},
        area_delivery_times: {},
        enable_delivery_hours: data.deliveryHoursEnabled,
        delivery_start_time: data.deliveryStartTime,
        delivery_end_time: data.deliveryEndTime,
        enable_same_day_delivery: data.sameDayDelivery,
        enable_next_day_delivery: data.nextDayDelivery,
        delivery_time_slots: data.deliveryTimeSlots?.map(slot => slot.time) || [],
        notify_customer_on_delivery: data.notifyCustomer,
        notify_driver_on_assignment: data.notifyDriver,
        enable_sms_notifications: data.sendSMS,
        enable_email_notifications: data.sendEmail,
        enable_driver_assignment: data.autoAssignDriver,
        driver_commission: data.driverCommission,
        require_signature: data.requireSignature,
        enable_driver_tracking: false,
        enable_scheduled_delivery: data.allowScheduledDelivery,
        enable_partial_delivery: data.allowPartialDelivery,
        require_advance_payment: data.requireAdvancePayment,
        advance_payment_percent: 50
      };

      await POSSettingsService.saveDeliverySettings(dbSettings);
