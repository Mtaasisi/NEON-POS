...
class PaymentTrackingService {
  ...
  private async checkConnection(): Promise<boolean> {
    ...
    try {
      ...
      // Try a simple auth check first (more reliable than table queries)
      const authResponse = await supabase.auth.getUser();
      if (!authResponse.data?.user) {
        this.connectionStatus = 'disconnected';
        console.warn('⚠️ PaymentTrackingService: No authenticated user');
        return false;
      }
      
      // If we have auth, assume connection is good
      // Database tables might have RLS policies preventing reads, but that's not a connection issue
      this.connectionStatus = 'connected';
      return true;
    } catch (error) {
      // On error, log but assume connected (fail-open strategy)
      console.debug('⚠️ PaymentTrackingService: Connection check error (assuming connected):', error);
      this.connectionStatus = 'connected';
      return true;
    }
  }
  ...
  async fetchSoldItems(transactionId: string, source: 'device_payment' | 'pos_sale' | 'purchase_order'): Promise<SoldItem[]> {
    try {
      if (source === 'pos_sale') {
  ...
  // Fetch all payment transactions from multiple sources (debounced)
    ...
    try {
      ...
      if (devicePaymentsError) {
        ...
        const transformedDevicePayments = devicePayments.map((payment: any) => ({
          ...
          netAmount: (Number(payment.amount) || 0) - (Number(payment.fees) || 0),
      ...
      try {
        ...
        if (!posSalesError && posSales && posSales.length > 0) {
          ...
          const transformedPOSSales = posSales.map((sale: any) => ({
            ...
            fees: Number(sale.processing_fees) || 0,
            netAmount: (Number(sale.total_amount) || 0) - (Number(sale.processing_fees) || 0),
            orderId: sale.id,
      ...
      try {
        ...
        if (!poPaymentsError && poPayments && poPayments.length > 0) {
          ...
          const transformedPOPayments = poPayments.map((payment: any) => ({
            ...
            customerName: 'Supplier Payment', // Simplified - supplier details can be fetched separately
            ...
            reference: payment.reference_number || payment.reference || `PO-REF-${payment.id.slice(0, 8).toUpperCase()}`,
            ...
            customerId: '', // Will be populated from separate query if needed
            paymentType: 'payment' as const,
            createdBy: payment.created_by,
            createdAt: payment.created_at,
            ...
            purchaseOrderNumber: undefined, // Can be fetched separately if needed
            supplierId: undefined, // Can be fetched separately if needed
            supplierName: undefined,
            supplierPhone: undefined,
            supplierEmail: undefined,
            paymentAccountId: payment.payment_account_id,
            paymentAccountName: undefined, // Can be fetched separately if needed
            notes: payment.notes
          }));
  ...
  async calculatePaymentMetrics(
    ...
    try {
      ...
      const completedAmount = payments.filter(p => p.status === 'completed').reduce((sum, payment) => sum + (Number(payment.amount) || 0), 0);
      const pendingAmount = payments.filter(p => p.status === 'pending').reduce((sum, payment) => sum + (Number(payment.amount) || 0), 0);
      const failedAmount = payments.filter(p => p.status === 'failed').reduce((sum, payment) => sum + (Number(payment.amount) || 0), 0);
      const totalFees = payments.reduce((sum, payment) => sum + (Number(payment.fees) || 0), 0);
    ...
    try {
      ...
      payments.forEach(payment => {
        ...
        methodMap.set(method, {
          ...
          amount: existing.amount + paymentAmount
        });
      });
  ...
  async getDailySummary(days: number = 7): Promise<DailySummary[]> {
    try {
      ...
      payments.forEach(payment => {
        ...
        existing.total += paymentAmount;
        if (payment.status === 'completed') existing.completed += paymentAmount;
        else if (payment.status === 'pending') existing.pending += paymentAmount;
        else if (payment.status === 'failed') existing.failed += paymentAmount;
  ...
  async getReconciliationRecords(): Promise<ReconciliationRecord[]> {
    try {
      ...
      payments.forEach(payment => {
        ...
        const paymentAmount = Number(payment.amount) || 0;
        dailyMap.set(date, existing + paymentAmount);
      });
  ...
  private mapPaymentMethod(method: any): string {
    ...
    // If method is an object, extract the actual method string
    if (typeof method === 'object') {
      // Try different possible property names
      const methodStr = method.method || method.paymentMethod || method.type;
    ...
    // Ensure method is a string before calling toLowerCase
