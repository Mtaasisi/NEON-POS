  ...
  static async uploadProductImage(
    ...
    try {
      ...
      try {
        ...
        if (user && !authError) {
          userId = user.id;
          console.log('✅ Using Supabase user:', userId);
        } else {
          // Fallback to localStorage user (Neon direct mode)
          const storedUser = localStorage.getItem('user');
          if (storedUser) {
            const parsedUser = JSON.parse(storedUser);
            userId = parsedUser.id || 'system';
            console.log('✅ Using localStorage user:', userId);
          } else {
            console.log('✅ Using system user (no auth required)');
          }
        }
      } catch (e) {
        console.log('⚠️ Auth check failed, using system user');
      }

      console.log('✅ Authentication passed (userId:', userId, ')');
