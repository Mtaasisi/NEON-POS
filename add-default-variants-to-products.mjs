...
/**
 * Add Default Variants to Products with 0 Variants
 * 
 * Adds a single "Default" variant to each product that currently has no variants.
 * 
 * Usage:
 *   node add-default-variants-to-products.mjs [--dry-run]
 */
...
async function createDefaultVariant(product) {
  ...
  // Ensure branch_id is set - use default branch if not set
  const branchId = product.branch_id || '00000000-0000-0000-0000-000000000001';
  ...
  const variantData = {
    ...
    min_quantity: product.min_stock_level || 5,
    branch_id: branchId,
    ...
    variant_attributes: {
      ...
      created_from: 'add_default_variants_script'
    },
    attributes: {}
  };
...
async function main() {
  try {
    console.log('🚀 Starting to add default variants to products with 0 variants...\n');
    ...
    // Show some examples
    console.log('📋 Examples of products that will get default variants:');
    productsWithoutVariants.slice(0, 10).forEach((p, i) => {
    ...
    for (let i = 0; i < productsWithoutVariants.length; i++) {
      ...
      if (i < productsWithoutVariants.length - 1 && i % 10 === 0) {
