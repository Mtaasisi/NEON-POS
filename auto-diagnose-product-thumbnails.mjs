...
async function takeProductScreenshots() {
  ...
  try {
    ...
    // Navigate to inventory page - try different routes
    ...
    for (const route of inventoryRoutes) {
      try {
        ...
        // Check if we found products on the page
        const productsFound = await page.evaluate(() => {
          // Look for product cards or product grids
          return document.querySelector('[class*="product"]') !== null ||
                 document.querySelector('[class*="Product"]') !== null ||
                 document.querySelector('img[alt*="product"]') !== null;
        });
        
        if (productsFound) {
          console.log(`   ✅ Found products at ${route}`);
          foundInventory = true;
          break;
        }
      } catch (e) {
    ...
    if (!foundInventory) {
      console.log('   ⚠️  No inventory route with products found, taking screenshot anyway');
    }
    
    // Try to switch to grid view if in list/table view
    try {
      console.log('   Checking for view mode toggle...');
      
      // Look for grid view button or toggle
      const gridViewButton = await page.$('[aria-label="Grid View"], button:has-text("Grid"), [title*="grid" i], [class*="grid" i]');
      if (gridViewButton) {
        console.log('   Found grid view button, clicking...');
        await gridViewButton.click();
        await page.waitForTimeout(2000);
      }
      
      // Alternative: look for view mode icons
      const viewModeButtons = await page.$$('button[class*="view"], button svg');
      if (viewModeButtons.length > 0) {
        console.log(`   Found ${viewModeButtons.length} view mode buttons, trying them...`);
        for (const btn of viewModeButtons.slice(0, 3)) {
          await btn.click();
          await page.waitForTimeout(1000);
          ...
          if (imageCount > 0) {
            console.log(`   ✅ Found view with ${imageCount} images!`);
            break;
          }
        }
      }
    } catch (e) {
      console.log(`   Note: Could not toggle view mode: ${e.message}`);
    }
