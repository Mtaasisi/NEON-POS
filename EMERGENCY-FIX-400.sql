...
BEGIN
  ...
  IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND information_schema.tables.table_name = 'lats_pos_notification_settings') THEN
...
BEGIN
  ...
  LOOP
    IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND information_schema.tables.table_name = tbl_name) THEN
      idx_name := 'idx_' || replace(tbl_name, 'lats_pos_', '') || '_user_id';
      EXECUTE format('CREATE INDEX IF NOT EXISTS %I ON %I(user_id)', idx_name, tbl_name);
      RAISE NOTICE '✅ Created index on %', tbl_name;
    END IF;
  END LOOP;
END $$;
