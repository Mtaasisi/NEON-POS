...
COMMENT ON TABLE daily_opening_sessions IS 
  'Tracks daily opening sessions for the POS system. RLS policies allow all authenticated users to access since sessions are system-wide, not user-specific. The opened_by_user_id column tracks who created each session for audit purposes.';
