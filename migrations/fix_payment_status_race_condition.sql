-- Fix payment status race condition in process_purchase_order_payment
-- The issue: When multiple payments are processed in parallel, they all read the same
-- initial v_po_paid value, causing incorrect status calculation.
-- Solution: Recalculate total_paid from all completed payments after inserting the new payment
