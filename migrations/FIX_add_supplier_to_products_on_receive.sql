-- =====================================================
-- FIX: Add supplier_id to products when receiving POs
-- =====================================================
-- This migration updates the purchase order receive function to set
-- supplier_id on products when they are received from a purchase order
      ...
      IF v_quantity > 0 THEN
        ...
        IF v_item_record.product_id IS NOT NULL AND v_order_record.supplier_id IS NOT NULL THEN
          ...
          WHERE id = v_item_record.product_id
            AND supplier_id IS NULL; -- Only update if supplier is not already set
        END IF;
...
'Completes the receive process for a purchase order by creating inventory items for all ordered items, updating product supplier_id, and updating the order status to received';

-- Update existing products that have NULL supplier_id but have purchase orders
-- This backfills supplier information for products that were already received
...
BEGIN
  ...
  RAISE NOTICE '✅ Updated % products with supplier information from their purchase orders', v_updated_count;
END $$;
