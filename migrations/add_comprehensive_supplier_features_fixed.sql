-- ================================================
-- COMPREHENSIVE SUPPLIER MANAGEMENT FEATURES (FIXED)
-- ================================================
-- Fixed version without auth.users foreign key references
-- ================================================
...
CREATE TABLE IF NOT EXISTS lats_supplier_documents (
  ...
  expiry_date DATE,
  notes TEXT,
  uploaded_by UUID, -- User ID without foreign key constraint
...
CREATE TABLE IF NOT EXISTS lats_supplier_communications (
  ...
  follow_up_date DATE,
  user_id UUID, -- User ID without foreign key constraint
  created_at TIMESTAMPTZ DEFAULT now(),
  updated_at TIMESTAMPTZ DEFAULT now()
);
...
CREATE TABLE IF NOT EXISTS lats_supplier_ratings (
  ...
  rated_by UUID, -- User ID without foreign key constraint
  created_at TIMESTAMPTZ DEFAULT now(),
  updated_at TIMESTAMPTZ DEFAULT now()
);
...
CREATE TABLE IF NOT EXISTS lats_supplier_contracts (
  ...
  notes TEXT,
  created_by UUID, -- User ID without foreign key constraint
  created_at TIMESTAMPTZ DEFAULT now(),
  updated_at TIMESTAMPTZ DEFAULT now()
);
...
WHERE s.is_active = true
  AND (s.is_trade_in_customer = false OR s.is_trade_in_customer IS NULL)
