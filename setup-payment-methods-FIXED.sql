-- ================================================================================
-- COMPLETE PAYMENT METHODS SETUP FOR POS - FIXED VERSION
-- ================================================================================
-- This script creates/fixes the finance_accounts table and sets up payment methods
-- Handles missing 'type' column gracefully
...
-- Add 'type' column if it doesn't exist (alias for account_type)
DO $$ 
BEGIN
  IF NOT EXISTS (
    ...
    RAISE NOTICE '✅ Added type column to finance_accounts';
  ELSE
    RAISE NOTICE '✅ type column already exists';
  END IF;
END $$;

-- Add 'is_payment_method' column if it doesn't exist
DO $$ 
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM information_schema.columns 
    ...
    RAISE NOTICE '✅ Added is_payment_method column to finance_accounts';
  ELSE
...
BEGIN
  IF NOT EXISTS (
    ...
    RAISE NOTICE '✅ Added is_active column to finance_accounts';
  ELSE
...
BEGIN
  IF NOT EXISTS (
    ...
    RAISE NOTICE '✅ Added icon column to finance_accounts';
  ELSE
...
BEGIN
  IF NOT EXISTS (
    ...
    RAISE NOTICE '✅ Added color column to finance_accounts';
  ELSE
...
DO $$
BEGIN
  -- Copy account_type to type if type is null
  UPDATE finance_accounts 
  SET type = account_type 
  WHERE type IS NULL AND account_type IS NOT NULL;
  
  -- Copy type to account_type if account_type is null
  UPDATE finance_accounts 
  SET account_type = type 
  WHERE account_type IS NULL AND type IS NOT NULL;
  
  RAISE NOTICE '✅ Synchronized type and account_type columns';
END $$;
...
-- ============================================================
-- 3. FIX EXISTING PAYMENT METHODS (if any)
-- ============================================================
...
UPDATE finance_accounts
SET 
    name = COALESCE(name, account_name),
...
UPDATE finance_accounts
SET 
    balance = COALESCE(balance::numeric, current_balance::numeric, 0),
    current_balance = COALESCE(current_balance::numeric, balance::numeric, 0)
WHERE is_payment_method = true;

-- Ensure type and account_type are in sync
UPDATE finance_accounts
SET 
    type = COALESCE(type, account_type),
...
-- ============================================================
-- 4. CREATE DEFAULT PAYMENT METHODS (if none exist)
-- ============================================================
...
-- ============================================================
-- 5. UPDATE ICONS AND COLORS FOR ALL PAYMENT METHODS
-- ============================================================
...
-- ============================================================
-- 6. GRANT PERMISSIONS (Neon compatible)
-- ============================================================
...
-- ============================================================
-- 7. DISABLE RLS (if it exists and causes issues)
-- ============================================================
