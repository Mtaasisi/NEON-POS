-- ============================================================================
-- APPLY DATABASE FIXES FOR VARIANT MERGING
-- ============================================================================
-- Run this script directly with psql to fix the variant merging issue
-- ============================================================================
...
-- Step 4: Fix existing duplicate variants for inauzwa product
DO $$
DECLARE
  v_product_id UUID;
...
SELECT 
  p.name,
  p.stock_quantity,
  COUNT(v.id) as total_variant_count,
  COUNT(CASE WHEN v.is_active = true THEN 1 END) as active_variant_count,
...
-- Step 6: Show final variant state
SELECT '=== FINAL VARIANT STATE ===' as status;
SELECT 
  v.id,
  v.name,
  v.variant_name,
